"""
Auth Middleware for Clinical Service.
"""
...
class PQCAuthMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next: RequestResponseEndpoint):
        if not request.url.path.startswith("/api/") and not request.url.path.startswith("/fhir/") and not request.url.path.startswith("/cds-services"):
