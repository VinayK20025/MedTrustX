...
import type { MaintenanceLog } from '../types/biomedTech.types';
import { FileText } from 'lucide-react';
import { cn } from '@/utils/cn';

interface Props { logs: MaintenanceLog[]; }

export function BiomedTechLogPanel({ logs }: Props) {
  return (
    <Card className="border-white/[0.06] shadow-glass bg-surface-light h-full flex flex-col">
      <CardHeader className="border-b border-white/[0.04] px-5 py-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-[15px] font-bold text-white tracking-wide">My Service Logs</h3>
        </div>
      </CardHeader>
              ...
              <div key={log.id} className="p-5 hover:bg-white/[0.015] transition-colors">
                <div className="flex justify-between items-start mb-2">
                  ...
                  <span className={cn('text-[9px] uppercase font-bold px-2 py-0.5 rounded tracking-wider', 
                    log.status === 'Successful' ? 'bg-success/20 text-success-light' : 'bg-warning/20 text-warning-light'
                  )}>
                    {log.status}
                  </span>
                </div>
                <p className="text-[11px] text-gray-400 italic mb-3">"{log.summary}"</p>
