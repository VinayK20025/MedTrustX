...
import type { ITOpsAlert } from '../types/itOps.types';
import { BellRing } from 'lucide-react';
import { Button } from '@/components/ui/Button';

interface Props { alerts: ITOpsAlert[]; }

export function ITOpsAlertPanel({ alerts }: Props) {
  return (
    <Card className="border-emergency/30 shadow-[0_0_20px_rgba(239,68,68,0.1)] bg-surface-light h-full flex flex-col">
      <CardHeader className="border-b border-emergency/20 px-5 py-3 flex items-center gap-2 bg-emergency/10">
        <BellRing className="w-4 h-4 text-emergency-light animate-pulse" />
        <h3 className="text-[15px] font-bold text-emergency-light tracking-wide">SYSTEM ALERTS</h3>
      </CardHeader>
        ...
        {alerts.map(a => (
          <div key={a.id} className={`p-3 rounded-lg border flex flex-col gap-2 ${
            ...
            <div className="flex justify-between items-center mt-1">
               ...
               <Button size="sm" variant="outline" className={`h-6 px-3 text-[10px] border-white/10 ${a.severity === 'critical' ? 'hover:bg-emergency/20 text-emergency-light' : 'hover:bg-warning/20 text-warning-light'}`}>Notify Teams</Button>
            </div>
          </div>
        ))}
      </CardBody>
