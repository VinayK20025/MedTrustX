...
import { ITOpsHealthPanel } from '../components/ITOpsHealthPanel';
import { ITOpsSLAPanel } from '../components/ITOpsSLAPanel';
import { ITOpsIncidentPanel } from '../components/ITOpsIncidentPanel';
import { ITOpsAlertPanel } from '../components/ITOpsAlertPanel';
import { useITOpsDashboard } from '../hooks/useITOpsAnalytics';
...
export function ITOpsDashboard() {
  const setPageMeta = useUIStore((s) => s.setPageMeta);
  const { data, isLoading } = useITOpsDashboard({});

  useEffect(() => { setPageMeta('IT Operations', 'System Health and SLA Oversight'); }, [setPageMeta]);
  ...
  return (
    <div className="space-y-5 animate-fade-in max-w-[1600px]">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <Breadcrumbs items={[{ label: 'IT Governance' }, { label: 'Ops Command Center' }]} />
        <div className="flex items-center gap-2 text-sm font-bold text-success-light bg-success/10 px-4 py-2 rounded-lg border border-success/30">
          <Activity className="w-4 h-4" /> ENTERPRISE HEALTH: NOMINAL
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-5 h-auto xl:h-[750px]">
        ...
        <div className="xl:col-span-7 h-[700px] xl:h-full flex flex-col gap-5">
           <div className="flex-1">
             <ITOpsHealthPanel healthData={d.health} />
           </div>
           <div className="flex-1">
             <ITOpsSLAPanel slas={d.slas} />
           </div>
        </div>
        ...
        <div className="xl:col-span-5 h-[600px] xl:h-full flex flex-col gap-5">
           <div className="flex-1">
             <ITOpsIncidentPanel incidents={d.incidents} />
           </div>
           <div className="h-[250px]">
             <ITOpsAlertPanel alerts={d.alerts} />
           </div>
        </div>
      </div>
    </div>
