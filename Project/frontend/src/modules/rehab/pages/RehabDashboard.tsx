...
import { RehabPatientPanel } from '../components/RehabPatientPanel';
import { RehabSessionPanel } from '../components/RehabSessionPanel';
import { RehabTherapyPanel } from '../components/RehabTherapyPanel';
import { RehabProgressPanel } from '../components/RehabProgressPanel';
import { RehabAssessmentPanel } from '../components/RehabAssessmentPanel';
import { RehabEducationPanel } from '../components/RehabEducationPanel';
import { useRehabDashboard } from '../hooks/useRehabAnalytics';
import type { RehabFilters } from '../services/rehab.api';
import type { RehabKPI } from '../types/rehab.types';
import { HeartPulse, TrendingUp, TrendingDown, ArrowRight } from 'lucide-react';
import { useRouter } from 'next/navigation';

function RehabKPICard({ kpi }: { kpi: RehabKPI }) {
...
export function RehabDashboard() {
  ...
  const [filters] = useState<RehabFilters>({ timeframe: 'today' });
  const { data, isLoading } = useRehabDashboard(filters);

  useEffect(() => {
    setPageMeta('Physiotherapist / OT', 'Rehabilitation tracking, therapy planning, and clinical progress');
  }, [setPageMeta]);
  ...
  return (
    <div className="space-y-5 animate-fade-in max-w-[1600px]">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <Breadcrumbs items={[{ label: 'Rehabilitation' }, { label: 'Therapist Workspace' }]} />
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {d.kpis.map(kpi => <RehabKPICard key={kpi.id} kpi={kpi} />)}
      </div>

      {/* Row 1: Active Patients & Daily Sessions */}
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-5">
        <div className="xl:col-span-7 h-[500px]">
          <RehabPatientPanel patients={d.patients} />
        </div>
        <div className="xl:col-span-5 h-[500px]">
          <RehabSessionPanel sessions={d.sessionsToday} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-5">
        <div className="xl:col-span-7 h-[400px]">
          <RehabProgressPanel metrics={d.progressMetrics} />
        </div>
        <div className="xl:col-span-5 h-[400px]">
          <RehabTherapyPanel plan={d.activePlan} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-5">
        <div className="xl:col-span-7 h-[350px]">
          <RehabAssessmentPanel assessments={d.assessments} />
        </div>
        <div className="xl:col-span-5 h-[350px]">
          <RehabEducationPanel materials={d.educationMaterials} />
        </div>
      </div>
    </div>
  );
