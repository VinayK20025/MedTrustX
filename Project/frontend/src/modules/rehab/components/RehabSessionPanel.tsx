...
import type { RehabSession } from '../types/rehab.types';
import { useStartSession, useCompleteSession } from '../hooks/useRehabAnalytics';
import { Calendar, Play, CheckCircle2, Clock } from 'lucide-react';
import { cn } from '@/utils/cn';

interface Props { sessions: RehabSession[]; }

export function RehabSessionPanel({ sessions }: Props) {
    ...
    <Card className="border-white/[0.06] shadow-glass bg-surface-light h-full flex flex-col">
      <CardHeader className="border-b border-white/[0.04] px-5 py-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-[15px] font-bold text-white tracking-wide">Today's Schedule</h3>
        </div>
      </CardHeader>
        ...
        <div className="divide-y divide-white/[0.03]">
          {sessions.map(sess => (
            <div key={sess.id} className={cn('p-5 transition-colors', sess.status === 'In Progress' ? 'bg-emerald-500/5 border-l-2 border-emerald-500' : 'hover:bg-white/[0.015]')}>
              ...
              <div className="bg-surface-dark border border-white/[0.04] p-3 rounded-lg mb-3 flex items-center justify-between">
                <div>
                  <p className="text-[11px] font-bold text-gray-300">{sess.type}</p>
                  <p className="text-[10px] text-gray-500 mt-1">Pre-session pain: <strong className="text-emergency-light">{sess.painLevelBefore}/10</strong></p>
                </div>
                {sess.status === 'Completed' && (
                  <div className="text-right">
                    <p className="text-[10px] text-gray-500">Post-session pain</p>
                    <p className="text-[12px] font-bold text-success-light">{sess.painLevelAfter}/10</p>
                  </div>
                )}
              </div>
              ...
              <div className="flex justify-end gap-2">
                ...
                {sess.status === 'In Progress' && (
                  <Button size="xs" onClick={() => completeSession({ sessionId: sess.id, notes: 'Completed standard routine' })} disabled={completing} className="h-7 text-[10px] bg-success hover:bg-success-light border-none font-bold" leftIcon={<CheckCircle2 className="w-3 h-3" />}>
                    Complete & Log
                  </Button>
                )}
              </div>
