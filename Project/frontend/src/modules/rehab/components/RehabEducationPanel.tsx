...
import type { RehabEducationMaterial } from '../types/rehab.types';
import { BookOpen, Video, FileText, Link as LinkIcon } from 'lucide-react';
import { cn } from '@/utils/cn';

interface Props { materials: RehabEducationMaterial[]; }

export function RehabEducationPanel({ materials }: Props) {
  return (
    <Card className="border-white/[0.06] shadow-glass bg-surface-light h-full flex flex-col">
      <CardHeader className="border-b border-white/[0.04] px-5 py-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-[15px] font-bold text-white tracking-wide">Patient Education</h3>
        </div>
      </CardHeader>
