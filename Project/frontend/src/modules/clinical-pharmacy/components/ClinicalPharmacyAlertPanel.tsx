...
import type { ClinicalAlert } from '../types/clinicalPharmacy.types';
import { AlertOctagon, Clock } from 'lucide-react';
import { cn } from '@/utils/cn';

interface Props { alerts: ClinicalAlert[]; }

export function ClinicalPharmacyAlertPanel({ alerts }: Props) {
    ...
    <Card className={cn("shadow-glass h-full flex flex-col bg-surface-light", active.length > 0 ? "border-emergency/40" : "border-white/[0.06]")}>
      <CardHeader className="border-b border-white/[0.04] px-5 py-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-[15px] font-bold text-white tracking-wide">Safety Alerts</h3>
          </div>
        </div>
      </CardHeader>
      ...
      <CardBody className="p-0 flex-1 overflow-y-auto max-h-[300px]">
        {active.length === 0 ? (
           <div className="p-8 text-center text-[12px] text-gray-500 font-bold">No active clinical pharmacy alerts.</div>
        ) : (
