...
import { NursingStaffPanel } from '../components/NursingStaffPanel';
import { NursingShiftPanel } from '../components/NursingShiftPanel';
import { NursingWardPanel } from '../components/NursingWardPanel';
import { NursingAlertsPanel } from '../components/NursingAlertsPanel';
import { useNursingDashboard } from '../hooks/useNursingAnalytics';
...
export function NursingSuperintendentDashboard() {
  const setPageMeta = useUIStore((s) => s.setPageMeta);
  const { data, isLoading } = useNursingDashboard({});

  useEffect(() => { setPageMeta('Nursing Command', 'Hospital-wide workforce orchestration and ward coverage'); }, [setPageMeta]);
  ...
  return (
    <div className="space-y-5 animate-fade-in max-w-[1600px]">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <Breadcrumbs items={[{ label: 'Nursing Superintendent' }, { label: 'Command Workspace' }]} />
        <div className="flex items-center gap-2 text-sm font-mono text-indigo-300 bg-indigo-500/10 px-3 py-1.5 rounded-lg border border-indigo-500/20">
          <Activity className="w-4 h-4" /> CURRENT SHIFT: MORNING
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-5 h-auto xl:h-[750px]">
        {/* Left: Workforce & Scheduling */}
        <div className="xl:col-span-8 h-[600px] xl:h-full flex flex-col gap-5">
           <div className="flex-1">
             <NursingShiftPanel schedules={d.schedules} />
           </div>
           <div className="h-[350px]">
             <NursingStaffPanel staff={d.staff} />
           </div>
        </div>
        
        {/* Right: Coverage & Alerts */}
        <div className="xl:col-span-4 h-[600px] xl:h-full flex flex-col gap-5">
           <div className="flex-1">
             <NursingWardPanel coverage={d.coverage} />
           </div>
           <div className="h-[300px]">
             <NursingAlertsPanel alerts={d.alerts} />
           </div>
        </div>
      </div>
    </div>
