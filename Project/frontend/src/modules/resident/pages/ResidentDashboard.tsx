...
import { ResidentPatientPanel } from '../components/ResidentPatientPanel';
import { ResidentAlertsPanel } from '../components/ResidentAlertsPanel';
import { useResidentDashboard } from '../hooks/useResidentAnalytics';
...
export function ResidentDashboard() {
  ...
  useEffect(() => { setPageMeta('Execution Workspace', 'Task-heavy execution and care delivery'); }, [setPageMeta]);
  ...
  return (
    <div className="space-y-5 animate-fade-in max-w-[1600px]">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <Breadcrumbs items={[{ label: 'Resident Doctor' }, { label: 'Dashboard' }]} />
        <div className="flex items-center gap-2 text-sm font-mono text-success-light bg-success/10 px-3 py-1.5 rounded-lg border border-success/20">
          <Activity className="w-4 h-4 animate-pulse" /> SHIFT ACTIVE
        </div>
      </div>
