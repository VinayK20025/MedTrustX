...
import { TriageIncomingQueue } from '../components/TriageIncomingQueue';
import { TriageAssessmentForm } from '../components/TriageAssessmentForm';
import { TriageRoutingPanel } from '../components/TriageRoutingPanel';
import { TriageAlertsPanel } from '../components/TriageAlertsPanel';
import { useTriageDashboard } from '../hooks/useTriageAnalytics';
...
export function TriageDashboard() {
  ...
  const { data, isLoading } = useTriageDashboard({});

  useEffect(() => { setPageMeta('Triage Nurse', 'Rapid emergency assessment and routing'); }, [setPageMeta]);
  ...
  return (
    <div className="space-y-5 animate-fade-in max-w-[1600px]">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <Breadcrumbs items={[{ label: 'Triage Nurse' }, { label: 'Intake Control' }]} />
        <div className="flex items-center gap-2 text-sm font-bold text-indigo-300 bg-indigo-500/10 px-4 py-2 rounded-lg border border-indigo-500/30">
          <Activity className="w-4 h-4" /> INTAKE: ACTIVE
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-5 h-auto xl:h-[750px]">
        {/* Left: Incoming Queue */}
        <div className="xl:col-span-4 h-[500px] xl:h-full flex flex-col gap-5">
           <div className="flex-1">
             <TriageIncomingQueue queue={d.incomingQueue || []} />
           </div>
        </div>
        
        {/* Middle: Assessment Form (CORE) */}
        ...
        {/* Right: Routing & Alerts */}
        <div className="xl:col-span-3 h-auto xl:h-full flex flex-col gap-5">
           <div className="flex-1">
             <TriageRoutingPanel options={d.routingOptions || []} />
           </div>
           <div className="h-[250px]">
             <TriageAlertsPanel alerts={d.alerts || []} />
           </div>
        </div>
      </div>
    </div>
