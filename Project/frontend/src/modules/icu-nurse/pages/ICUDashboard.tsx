...
import { ICUPatientGrid } from '../components/ICUPatientGrid';
import { ICUAlertsPanel } from '../components/ICUAlertsPanel';
import { ICUTaskPanel } from '../components/ICUTaskPanel';
import { useICUDashboard } from '../hooks/useICUAnalytics';
...
export function ICUDashboard() {
  ...
  const { data, isLoading } = useICUDashboard({});

  useEffect(() => { setPageMeta('ICU Workspace', 'Real-time continuous monitoring and life-support execution'); }, [setPageMeta]);
  ...
  return (
    <div className="space-y-5 animate-fade-in max-w-[1600px]">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <Breadcrumbs items={[{ label: 'ICU Nurse' }, { label: 'Live Monitoring' }]} />
        <div className="flex items-center gap-2 text-sm font-mono text-emergency-light bg-emergency/10 px-3 py-1.5 rounded-lg border border-emergency/20">
          <Activity className="w-4 h-4 animate-pulse" /> LIVE STREAM: ACTIVE
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-5 h-auto xl:h-[750px]">
        {/* Left: Patient Grid (CORE) */}
        <div className="xl:col-span-8 h-[600px] xl:h-full flex flex-col gap-5">
           <div className="flex-1">
             <ICUPatientGrid patients={d.patients} />
           </div>
        </div>
        
        {/* Right: Critical Alerts & Interventions */}
        <div className="xl:col-span-4 h-auto xl:h-full flex flex-col gap-5">
           <div className="h-[350px]">
             <ICUAlertsPanel alerts={d.alerts} />
           </div>
           <div className="flex-1">
             <ICUTaskPanel tasks={d.tasks} />
           </div>
        </div>
      </div>
    </div>
