...
import type { PlumbingTask, PlumTaskPriority } from '../types/plumber.types';
import { Droplets, AlertTriangle, MapPin } from 'lucide-react';
import { cn } from '@/utils/cn';

interface Props { tasks: PlumbingTask[]; selectedId?: string; onSelect: (id: string) => void; }

const priorityColor: Record<PlumTaskPriority, string> = {
  Routine: 'bg-gray-500/20 text-gray-300',
  Medium: 'bg-orange-500/20 text-orange-400',
  Critical: 'bg-emergency/20 text-emergency-light border border-emergency/30',
};

export function PlumberTaskQueue({ tasks, selectedId, onSelect }: Props) {
  return (
    <Card className="border-white/[0.06] shadow-glass bg-surface-light h-full flex flex-col">
      <CardHeader className="border-b border-white/[0.04] px-4 py-3 flex items-center gap-2">
        <div className="p-1.5 rounded bg-cyan-500/15"><Droplets className="w-4 h-4 text-cyan-400" /></div>
        <h3 className="text-[14px] font-bold text-white tracking-wide">Plumbing Tasks</h3>
      </CardHeader>
          ...
          {tasks.map(t => (
            <div key={t.id} onClick={() => onSelect(t.id)}
              className={cn("p-4 cursor-pointer transition-all border-l-4 relative",
                selectedId === t.id ? "bg-cyan-500/[0.08] border-l-cyan-500" :
              ...
              <div className="flex justify-between items-center mt-3">
                <span className={cn("text-[10px] font-bold px-2 py-1 rounded", 
                  t.status === 'In Progress' ? 'bg-cyan-500/20 text-cyan-400' : 'bg-white/10 text-gray-300'
                )}>{t.status}</span>
