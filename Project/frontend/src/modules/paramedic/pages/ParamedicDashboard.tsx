...
import { ParamedicVitalsPanel } from '../components/ParamedicVitalsPanel';
import { useParamedicDashboard } from '../hooks/useParamedicAnalytics';
import type { ParamedicKPI } from '../types/paramedic.types';
import { Stethoscope, ShieldAlert } from 'lucide-react';

function ParaKPICard({ kpi }: { kpi: ParamedicKPI }) {
...
export function ParamedicDashboard() {
  ...
  const { data, isLoading } = useParamedicDashboard({});

  useEffect(() => {
    setPageMeta('Ambulance Paramedic', 'Pre-hospital clinical assessment, rapid interventions, and live ER data sync');
  }, [setPageMeta]);
  ...
  const isCriticalPatient = d.activeCase && d.activeCase.triageLevel === 'Critical';
  ...
  return (
    <div className="space-y-4 animate-fade-in max-w-[1600px] mx-auto">
      ...
      {isCriticalPatient && (
        <div className="bg-emergency/20 border border-emergency/40 rounded-xl px-5 py-3 flex items-center gap-3 animate-pulse">
          ...
          <div className="flex-1">
            <span className="text-[12px] font-black text-emergency-light uppercase tracking-widest">CRITICAL PATIENT ONBOARD</span>
            <p className="text-[11px] text-red-300 mt-0.5">Patient requires immediate life-saving interventions. Ensure "Severe Trauma Protocol" is initiated and ER is notified.</p>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <Breadcrumbs items={[{ label: 'Emergency Services' }, { label: 'Clinical Response' }]} />
        <div className="text-[12px] font-bold text-cyan-400 flex items-center gap-2 bg-cyan-500/10 px-4 py-2 rounded-lg border border-cyan-500/25">
          <Stethoscope className="w-4 h-4" /> Live Protocol Support Active
        </div>
      </div>

      {/* KPIs */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {d.kpis.map(k => <ParaKPICard key={k.id} kpi={k} />)}
      </div>

      {/* Main Layout */}
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-5 h-[calc(100vh-290px)] min-h-[600px]">
        <div className="xl:col-span-3 h-full">
          <ActiveCasePanel activeCase={d.activeCase} />
        </div>
        <div className="xl:col-span-6 h-full">
          <ClinicalWorkspace protocols={d.protocols} interventions={d.interventions} />
        </div>
        <div className="xl:col-span-3 h-full">
          <ParamedicVitalsPanel vitalsHistory={d.vitalsHistory} />
        </div>
      </div>
    </div>
  );
