...
import type { CioAlert } from '../types/cio.types';
import { useResolveCioAlert } from '../hooks/useCioAnalytics';

interface CioAlertsPanelProps {
  alerts: CioAlert[];
}

export function CioAlertsPanel({ alerts }: CioAlertsPanelProps) {
  const { mutate: resolveAlert, isPending } = useResolveCioAlert();
  ...
  return (
    <Card className="border-white/[0.06] shadow-glass bg-surface-light h-full flex flex-col font-mono">
      <CardHeader className="border-b border-white/[0.04] px-5 py-4 flex items-center justify-between font-sans">
        <div>
          <h3 className="text-lg font-semibold text-white tracking-wide">System Alerts</h3>
          <p className="text-xs text-gray-400 mt-0.5">Automated infrastructure & service flags</p>
        </div>
      </CardHeader>
      ...
      <CardBody className="p-0 flex-1 overflow-y-auto max-h-[400px]">
        <div className="divide-y divide-white/[0.04]">
          ...
          {alerts.length === 0 && (
            <div className="p-8 text-center text-gray-500">No active system alerts</div>
          )}
        </div>
      </CardBody>
    </Card>
