...
import type { SpeechEducationMaterial } from '../types/speech.types';
import { BookOpen, Video, FileText, Link as LinkIcon } from 'lucide-react';

interface Props { materials: SpeechEducationMaterial[]; }

export function SpeechEducationPanel({ materials }: Props) {
  return (
    <Card className="border-white/[0.06] shadow-glass bg-surface-light h-full flex flex-col">
      <CardHeader className="border-b border-white/[0.04] px-5 py-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-[15px] font-bold text-white tracking-wide">Patient Guidance</h3>
        </div>
      </CardHeader>
