...
import { PlatformClusterPanel } from '../components/PlatformClusterPanel';
import { PlatformWorkloadPanel } from '../components/PlatformWorkloadPanel';
import { PlatformObservabilityPanel } from '../components/PlatformObservabilityPanel';
import { PlatformAlertPanel } from '../components/PlatformAlertPanel';
import { usePlatformDashboard } from '../hooks/usePlatformAnalytics';
import type { PlatformFilters } from '../services/platform.api';
import type { PlatformKPI } from '../types/platform.types';
...
export function PlatformDashboard() {
  ...
  useEffect(() => {
    setPageMeta('Platform Engineer', 'Kubernetes cluster orchestration, workload scaling, and Prometheus observability');
  }, [setPageMeta]);
  ...
  return (
    <div className="space-y-5 animate-fade-in max-w-[1600px]">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <Breadcrumbs items={[{ label: 'Infrastructure' }, { label: 'Platform Engineering' }]} />
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {d.kpis.map(kpi => <PlatformKPICard key={kpi.id} kpi={kpi} />)}
      </div>

      {/* Row 1: Observability */}
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-5">
        <div className="xl:col-span-12 h-[200px]">
          <PlatformObservabilityPanel metrics={d.metrics} />
        </div>
      </div>

      {/* Row 2: Clusters & Workloads */}
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-5">
        <div className="xl:col-span-6 h-[450px]">
          <PlatformClusterPanel clusters={d.clusters} nodes={d.nodes} />
        </div>
        <div className="xl:col-span-6 h-[450px]">
          <PlatformWorkloadPanel workloads={d.workloads} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-5">
        <div className="xl:col-span-12 h-[300px]">
          <PlatformAlertPanel alerts={d.alerts} />
        </div>
      </div>
    </div>
  );
