...
import type { RadiologyAlert } from '../types/radiology.types';
import { AlertTriangle, Clock, Bot } from 'lucide-react';
import { cn } from '@/utils/cn';

interface Props { alerts: RadiologyAlert[]; }

export function RadiologyAlertPanel({ alerts }: Props) {
    ...
    <Card className={cn("shadow-glass h-full flex flex-col bg-surface-light", active.length > 0 ? "border-emergency/40" : "border-white/[0.06]")}>
      <CardHeader className="border-b border-white/[0.04] px-5 py-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-[15px] font-bold text-white tracking-wide">Triage & Alerts</h3>
          </div>
        </div>
      </CardHeader>
      ...
      <CardBody className="p-0 flex-1 overflow-y-auto max-h-[300px]">
        {active.length === 0 ? (
           <div className="p-8 text-center text-[12px] text-gray-500 font-bold">No urgent cases. Queue is nominal.</div>
        ) : (
              ...
              <div key={alert.id} className={cn("px-5 py-4 border-l-2", alert.severity === 'critical' ? 'bg-emergency/[0.05] border-emergency' : 'bg-warning/[0.05] border-warning')}>
                <div className="flex justify-between items-start mb-2">
                  <h4 className={cn("text-[13px] font-bold flex items-center gap-2", alert.severity === 'critical' ? 'text-emergency-light' : 'text-warning-light')}>
                    {alert.type === 'AI Anomaly Detected' && <Bot className="w-3.5 h-3.5" />}
                    {alert.type}
                  </h4>
                  {alert.studyId && <span className="text-[10px] bg-white/5 px-1.5 py-0.5 rounded text-gray-400 font-mono">{alert.studyId}</span>}
                </div>
