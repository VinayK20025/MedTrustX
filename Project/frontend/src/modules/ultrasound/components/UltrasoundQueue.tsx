...
import type { UltrasoundQueue } from '../types/ultrasound.types';
import { Users, AlertTriangle, Clock } from 'lucide-react';
import { cn } from '@/utils/cn';

interface Props { queue: UltrasoundQueue[]; }

export function UltrasoundQueueList({ queue }: Props) {
  return (
    <Card className="border-white/[0.06] shadow-glass bg-surface-light h-full flex flex-col">
      <CardHeader className="border-b border-white/[0.04] px-5 py-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-[15px] font-bold text-white tracking-wide">Ultrasound Queue</h3>
        </div>
      </CardHeader>
              ...
              <div className="flex justify-between items-center mt-4 border-t border-white/[0.04] pt-3">
                <div className="flex gap-2">
                  <span className={cn("text-[10px] uppercase font-bold px-2 py-1 rounded border", 
                     p.status === 'Scanning' ? 'text-teal-400 border-teal-500/30 bg-teal-500/10' : 'text-gray-400 border-white/5'
                  )}>{p.status}</span>
                </div>
