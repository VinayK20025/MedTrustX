...
import type { ImagingPatientQueue } from '../types/radiologyTech.types';
import { Users, AlertTriangle, Clock } from 'lucide-react';
import { cn } from '@/utils/cn';

interface Props { queue: ImagingPatientQueue[]; }

export function RadiologyTechQueue({ queue }: Props) {
  return (
    <Card className="border-white/[0.06] shadow-glass bg-surface-light h-full flex flex-col">
      <CardHeader className="border-b border-white/[0.04] px-5 py-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div className="p-2 rounded-lg bg-orange-500/15"><Users className="w-4 h-4 text-orange-400" /></div>
          <h3 className="text-[15px] font-bold text-white tracking-wide">Imaging Queue</h3>
        </div>
      </CardHeader>
            ...
            <div key={p.id} className={cn("p-5 transition-colors cursor-pointer", p.priority === 'STAT' ? "bg-emergency/5 border-l-2 border-emergency" : "hover:bg-white/[0.015]")}>
              ...
              <div className="flex justify-between items-center mt-4 border-t border-white/[0.04] pt-3">
                <div className="flex gap-2">
                  <span className="text-[10px] uppercase font-bold text-gray-300 bg-surface-dark px-2 py-1 rounded border border-white/10">{p.modality}</span>
                  <span className={cn("text-[10px] uppercase font-bold px-2 py-1 rounded border", 
                     p.status === 'In Setup' ? 'text-orange-400 border-orange-500/30 bg-orange-500/10' : 'text-gray-400 border-white/5'
                  )}>{p.status}</span>
                </div>
