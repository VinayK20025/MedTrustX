...
import { LitigationCasesPanel } from '../components/LitigationCasesPanel';
import { HearingsPanel } from '../components/HearingsPanel';
import { LegalPartiesPanel } from '../components/LegalPartiesPanel';
import { LitigationUpdatesPanel } from '../components/LitigationUpdatesPanel';
import { useLitigationTracking } from '../hooks/useLitigationTracking';
import { Scale, Calendar, Users, FileSignature, AlertCircle, DollarSign } from 'lucide-react';
...
function LitKPICard({ kpi }: { kpi: LitKPI }) {
...
export const LitigationTrackingDashboard: React.FC = () => {
  const setPageMeta = useUIStore((s) => s.setPageMeta);
  const { useCases } = useLitigationTracking();
  const casesQuery = useCases();

  useEffect(() => {
    setPageMeta('Litigation Tracking', 'Docket management, court hearings, and settlement negotiations');
  }, [setPageMeta]);
  ...
  const kpis: LitKPI[] = [
    { id: 'docket', title: 'Active Docket', value: 18, status: 'normal', icon: Scale, subtitle: 'Court cases' },
    { id: 'hearings', title: 'Upcoming Hearings', value: 4, status: 'warning', icon: Calendar, subtitle: 'Next 30 days' },
    { id: 'parties', title: 'Legal Parties', value: 52, status: 'normal', icon: Users, subtitle: 'Plaintiffs & Counsel' },
    { id: 'settlements', title: 'Settlements', value: 2, status: 'success', icon: FileSignature, subtitle: 'Negotiations pending' },
    { id: 'deadlines', title: 'Missed Deadlines', value: 0, status: 'success', icon: AlertCircle, subtitle: 'Filing compliance' },
    { id: 'reserves', title: 'Legal Reserves', value: '$1.2M', status: 'normal', icon: DollarSign, subtitle: 'Held for claims' },
  ];
  ...
  return (
    <div className="space-y-5 animate-fade-in max-w-[1600px]">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        {kpis.map(kpi => <LitKPICard key={kpi.id} kpi={kpi} />)}
      </div>
