...
import type { SREAlert } from '../types/sre.types';
import { Siren } from 'lucide-react';
import { Button } from '@/components/ui/Button';

interface Props { alerts: SREAlert[]; }

export function SREAlertPanel({ alerts }: Props) {
  return (
    <Card className="border-warning/30 shadow-[0_0_20px_rgba(245,158,11,0.1)] bg-surface-light h-full flex flex-col">
      <CardHeader className="border-b border-warning/20 px-5 py-3 flex items-center gap-2 bg-warning/10">
        <Siren className="w-4 h-4 text-warning-light animate-pulse" />
        <h3 className="text-[15px] font-bold text-warning-light tracking-wide">SYSTEM ALERTS</h3>
      </CardHeader>
