import React from 'react';
import { Card, CardHeader, CardBody } from '@/components/ui/Card';
import { Badge } from '@/components/ui/Badge';
import { useSecurityIncident } from '../hooks/useSecurityIncident';
import type { IncidentAction } from '../types/security-incident.types';

export const IncidentActionsPanel: React.FC = () => {
  const { useActions } = useSecurityIncident();
  const { data: response, isLoading } = useActions();

  const actions = response?.data || [
    { id: '1', incident_id: '1', action_type: 'lockdown_zone', status: 'completed', performed_at: new Date().toISOString() },
    { id: '2', incident_id: '1', action_type: 'deploy_cctv_override', status: 'initiated', performed_at: new Date().toISOString() }
  ];

  if (isLoading) return <div>Loading actions...</div>;

  return (
    <Card className="h-full">
      <CardHeader title="Response Actions" />
      <CardBody>
        <ul className="border-l-2 border-white/10 ml-3 space-y-4">
          {actions.map((a: IncidentAction) => (
            <li key={a.id} className="relative pl-4">
              <div className={`absolute -left-[5px] top-1.5 w-2 h-2 rounded-full ${a.status === 'completed' ? 'bg-emerald-500' : 'bg-amber-500'}`} />
              <div className="flex items-center gap-2 mb-1">
                <span className="text-sm font-semibold capitalize">{a.action_type.replace(/_/g, ' ')}</span>
                <Badge variant={a.status === 'completed' ? 'success' : 'warning'}>
                  {a.status.toUpperCase()}
                </Badge>
              </div>
              <p className="text-xs text-gray-500">{new Date(a.performed_at).toLocaleString()}</p>
            </li>
          ))}
        </ul>
      </CardBody>
    </Card>
  );
};
