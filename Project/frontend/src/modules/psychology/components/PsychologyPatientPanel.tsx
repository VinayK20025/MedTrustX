...
import type { PsychologyPatient } from '../types/psychology.types';
import { Users, BrainCircuit, AlertTriangle, ArrowRight } from 'lucide-react';
import { cn } from '@/utils/cn';

interface Props { patients: PsychologyPatient[]; }

export function PsychologyPatientPanel({ patients }: Props) {
  return (
    <Card className="border-white/[0.06] shadow-glass bg-surface-light h-full flex flex-col">
      <CardHeader className="border-b border-white/[0.04] px-5 py-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-[15px] font-bold text-white tracking-wide">Active Clients</h3>
        </div>
          ...
          {patients.map(patient => (
            <div key={patient.id} className="p-5 hover:bg-white/[0.015] transition-colors group cursor-pointer">
              ...
              <div className="grid grid-cols-2 gap-3 bg-surface-dark border border-white/[0.04] p-3 rounded-lg mb-3">
                <div>
                  <span className="text-[9px] font-bold text-gray-500 uppercase flex items-center gap-1"><BrainCircuit className="w-3 h-3" /> Therapy Focus</span>
                  <div className="mt-1 flex items-center gap-2">
                    <span className="text-sm font-bold text-indigo-400 truncate">Cognitive Restructuring</span>
                  </div>
                </div>
                <div>
                  <span className="text-[9px] font-bold text-gray-500 uppercase flex items-center gap-1"><AlertTriangle className="w-3 h-3 text-warning-light" /> Risk Level</span>
                  <p className={cn("text-[10px] font-bold mt-1", 
                    patient.riskLevel === 'High' || patient.riskLevel === 'Severe' ? 'text-emergency-light' : 
                    patient.riskLevel === 'Moderate' ? 'text-warning-light' : 'text-success-light'
                  )}>{patient.riskLevel}</p>
                </div>
              </div>
