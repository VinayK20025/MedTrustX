...
import type { ICNAlert } from '../types/icn.types';
import { Siren } from 'lucide-react';
import { Button } from '@/components/ui/Button';

interface Props { alerts: ICNAlert[]; }

export function ICNAlertsPanel({ alerts }: Props) {
  return (
        ...
        {alerts.map(a => (
          <div key={a.id} className={`p-3 rounded-lg border flex flex-col gap-2 ${
            ...
            <div className="flex justify-between items-center mt-1">
               ...
               <Button size="sm" variant="outline" className={`h-6 px-3 text-[10px] border-white/10 ${a.severity === 'critical' ? 'hover:bg-emergency/20 text-emergency-light' : 'hover:bg-warning/20 text-warning-light'}`}>Investigate</Button>
            </div>
          </div>
        ))}
      </CardBody>
