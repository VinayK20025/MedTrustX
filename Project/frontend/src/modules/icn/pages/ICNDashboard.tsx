...
import { ICNActiveCases } from '../components/ICNActiveCases';
import { ICNOutbreakInvestigation } from '../components/ICNOutbreakInvestigation';
import { ICNAnalyticsPanel } from '../components/ICNAnalyticsPanel';
import { ICNAlertsPanel } from '../components/ICNAlertsPanel';
...
export function ICNDashboard() {
  ...
  useEffect(() => { setPageMeta('ICN Surveillance', 'Hospital-wide infection and compliance tracking'); }, [setPageMeta]);
  ...
  return (
    <div className="space-y-5 animate-fade-in max-w-[1600px]">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <Breadcrumbs items={[{ label: 'Infection Control' }, { label: 'Surveillance Hub' }]} />
        <div className="flex items-center gap-2 text-sm font-bold text-success-light bg-success/10 px-4 py-2 rounded-lg border border-success/30">
          <ShieldCheck className="w-4 h-4" /> SURVEILLANCE: ACTIVE
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-5 h-auto xl:h-[750px]">
        {/* Left: Active Cases & Analytics */}
        <div className="xl:col-span-7 h-[700px] xl:h-full flex flex-col gap-5">
           <div className="flex-1">
             <ICNActiveCases cases={d.activeCases} />
           </div>
           <div className="flex-1">
             <ICNAnalyticsPanel analytics={d.analytics} />
           </div>
        </div>
        
        {/* Right: Outbreaks & Alerts */}
        <div className="xl:col-span-5 h-auto xl:h-full flex flex-col gap-5">
           <div className="h-[350px]">
             <ICNOutbreakInvestigation outbreaks={d.outbreaks} />
           </div>
           <div className="flex-1">
             <ICNAlertsPanel alerts={d.alerts} />
           </div>
        </div>
      </div>
    </div>
