...
import { NurseTaskPanel } from '../components/NurseTaskPanel';
import { NursePatientPanel } from '../components/NursePatientPanel';
import { NurseAlertsPanel } from '../components/NurseAlertsPanel';
import { useNurseDashboard } from '../hooks/useNurseAnalytics';
...
export function NurseDashboard() {
  ...
  useEffect(() => { setPageMeta('Nurse Workspace', 'Bedside task execution and patient care monitoring'); }, [setPageMeta]);
  ...
  return (
    <div className="space-y-5 animate-fade-in max-w-[1600px]">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <Breadcrumbs items={[{ label: 'Staff Nurse' }, { label: 'Care Execution' }]} />
        <div className="flex items-center gap-2 text-sm font-mono text-success-light bg-success/10 px-3 py-1.5 rounded-lg border border-success/20">
          <ShieldCheck className="w-4 h-4" /> SECURE SHIFT: ACTIVE
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-5 h-auto xl:h-[750px]">
        ...
        <div className="xl:col-span-4 h-auto xl:h-full flex flex-col gap-5">
           <div className="flex-1">
             <NursePatientPanel patients={d.patients} />
           </div>
           <div className="flex-1">
