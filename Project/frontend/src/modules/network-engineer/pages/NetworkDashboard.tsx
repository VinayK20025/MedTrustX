...
import { NetworkSegmentationPanel } from '../components/NetworkSegmentationPanel';
import { NetworkZeroTrustPanel } from '../components/NetworkZeroTrustPanel';
import { NetworkTrafficPanel } from '../components/NetworkTrafficPanel';
import { NetworkAlertPanel } from '../components/NetworkAlertPanel';
...
export function NetworkDashboard() {
  ...
  useEffect(() => {
    setPageMeta('Network Engineer', 'VPC topology, Micro-segmentation, and Zero Trust access management');
  }, [setPageMeta]);
  ...
  return (
    <div className="space-y-5 animate-fade-in max-w-[1600px]">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <Breadcrumbs items={[{ label: 'Infrastructure Security' }, { label: 'Network Engineer' }]} />
      ...
      {/* Row 1: Traffic Flow Logs (Terminal Style) */}
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-5">
        <div className="xl:col-span-12 h-[250px]">
          <NetworkTrafficPanel flows={d.traffic} />
        </div>
      </div>

      {/* Row 2: Topology & Segmentation */}
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-5">
        <div className="xl:col-span-7 h-[450px]">
          <NetworkTopologyPanel vpcs={d.vpcs} subnets={d.subnets} />
        </div>
        <div className="xl:col-span-5 h-[450px]">
          <NetworkSegmentationPanel policies={d.segmentation} />
        </div>
      </div>

      {/* Row 3: Identity & Alerts */}
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-5">
        <div className="xl:col-span-6 h-[300px]">
          <NetworkZeroTrustPanel identities={d.zeroTrust} />
        </div>
        <div className="xl:col-span-6 h-[300px]">
          <NetworkAlertPanel alerts={d.alerts} />
        </div>
      </div>
    </div>
  );
