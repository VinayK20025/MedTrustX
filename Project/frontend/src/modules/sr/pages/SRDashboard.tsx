...
import { SRTaskBoard } from '../components/SRTaskBoard';
import { SRTeamPanel } from '../components/SRTeamPanel';
import { SRPatientPanel } from '../components/SRPatientPanel';
import { SRAlertsPanel } from '../components/SRAlertsPanel';
import { useSRDashboard } from '../hooks/useSRAnalytics';
...
export function SRDashboard() {
  ...
  useEffect(() => { setPageMeta('Operations Control', 'Team supervision and clinical delegation'); }, [setPageMeta]);
  ...
  return (
    <div className="space-y-5 animate-fade-in max-w-[1600px]">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <Breadcrumbs items={[{ label: 'Senior Resident' }, { label: 'Dashboard' }]} />
        <div className="flex items-center gap-2 text-sm font-mono text-indigo-300 bg-indigo-500/10 px-3 py-1.5 rounded-lg border border-indigo-500/20">
          <Activity className="w-4 h-4 animate-pulse" /> WARD: GEN MED & PULMONARY
        </div>
      </div>
