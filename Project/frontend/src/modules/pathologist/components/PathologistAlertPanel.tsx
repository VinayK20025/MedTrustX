...
import type { PathologistAlert } from '../types/pathologist.types';
import { AlertTriangle, Clock } from 'lucide-react';
import { cn } from '@/utils/cn';

interface Props { alerts: PathologistAlert[]; }

export function PathologistAlertPanel({ alerts }: Props) {
    ...
    <Card className={cn("shadow-glass h-full flex flex-col bg-surface-light", active.length > 0 ? "border-emergency/40" : "border-white/[0.06]")}>
      <CardHeader className="border-b border-white/[0.04] px-5 py-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-[15px] font-bold text-white tracking-wide">Critical Lab Alerts</h3>
          </div>
        </div>
      </CardHeader>
      ...
      <CardBody className="p-0 flex-1 overflow-y-auto max-h-[300px]">
        {active.length === 0 ? (
           <div className="p-8 text-center text-[12px] text-gray-500 font-bold">No critical value alerts.</div>
        ) : (
          <div className="divide-y divide-white/[0.03]">
            {active.map(alert => (
              <div key={alert.id} className={cn("px-5 py-4 border-l-2", alert.severity === 'critical' ? 'bg-emergency/[0.05] border-emergency' : 'bg-warning/[0.05] border-warning')}>
                ...
                <p className="text-[11px] text-gray-300 mb-1">{alert.message}</p>
                <div className="flex justify-between items-center mt-2">
                  <span className="text-[10px] font-bold text-white">{alert.patientName}</span>
