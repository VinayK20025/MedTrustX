...
import type { BiochemAlert } from '../types/biochemistry.types';
import { AlertTriangle, Clock } from 'lucide-react';
import { cn } from '@/utils/cn';

interface Props { alerts: BiochemAlert[]; }

export function BiochemAlertPanel({ alerts }: Props) {
    ...
    <Card className={cn("shadow-glass h-full flex flex-col bg-surface-light", active.length > 0 ? "border-emergency/40" : "border-white/[0.06]")}>
      <CardHeader className="border-b border-white/[0.04] px-5 py-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-[15px] font-bold text-white tracking-wide">QC & Critical Alerts</h3>
          </div>
        </div>
      </CardHeader>
      ...
      <CardBody className="p-0 flex-1 overflow-y-auto max-h-[300px]">
        {active.length === 0 ? (
           <div className="p-8 text-center text-[12px] text-gray-500 font-bold">No active alerts. QC is nominal.</div>
        ) : (
            ...
            {active.map(alert => (
              <div key={alert.id} className={cn("px-5 py-4 border-l-2", alert.severity === 'critical' ? 'bg-emergency/[0.05] border-emergency' : 'bg-warning/[0.05] border-warning')}>
                <div className="flex justify-between items-start mb-2">
                  ...
                  {alert.sampleId && <span className="text-[10px] bg-white/5 px-1.5 py-0.5 rounded text-gray-400 font-mono">{alert.sampleId}</span>}
                  {alert.analyzerId && <span className="text-[10px] bg-white/5 px-1.5 py-0.5 rounded text-gray-400 font-mono">{alert.analyzerId}</span>}
                </div>
