...
import { IamUserPanel } from '../components/IamUserPanel';
import { IamRolePanel } from '../components/IamRolePanel';
import { IamPolicyPanel } from '../components/IamPolicyPanel';
import { IamAuthPanel } from '../components/IamAuthPanel';
import { IamRequestPanel } from '../components/IamRequestPanel';
import { IamReviewPanel } from '../components/IamReviewPanel';
import { IamAuditPanel } from '../components/IamAuditPanel';
import { useIamDashboard } from '../hooks/useIamAnalytics';
import type { IAMFilters } from '../services/iam.api';
import type { IAMKPI } from '../types/iam.types';
import { Shield, TrendingUp, TrendingDown, ArrowRight } from 'lucide-react';
import { useRouter } from 'next/navigation';

function IamKPICard({ kpi }: { kpi: IAMKPI }) {
...
export function IamDashboard() {
  ...
  const [filters] = useState<IAMFilters>({ timeframe: 'today' });
  const { data, isLoading } = useIamDashboard(filters);

  useEffect(() => {
    setPageMeta('IAM Administrator', 'Identity governance, access control, and Zero Trust policies');
  }, [setPageMeta]);
  ...
  return (
    <div className="space-y-5 animate-fade-in max-w-[1600px]">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <Breadcrumbs items={[{ label: 'Platform' }, { label: 'IAM Workspace' }]} />
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {d.kpis.map(kpi => <IamKPICard key={kpi.id} kpi={kpi} />)}
      </div>

      {/* Row 1: Users, Roles, Policies */}
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-5">
        <div className="xl:col-span-5 h-[450px]">
          <IamUserPanel identities={d.identities} />
        </div>
        <div className="xl:col-span-4 h-[450px]">
          <IamPolicyPanel policies={d.policies} />
        </div>
        <div className="xl:col-span-3 h-[450px]">
          <IamRolePanel roles={d.roles} />
        </div>
      </div>

      {/* Row 2: Auth, Requests, Reviews, Audit */}
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-5">
        <div className="lg:col-span-1 h-[400px]">
          <IamAuthPanel authMethods={d.authMethods} />
        </div>
        <div className="lg:col-span-1 h-[400px]">
          <IamRequestPanel requests={d.requests} />
        </div>
        <div className="lg:col-span-1 h-[400px]">
          <IamReviewPanel reviews={d.reviews} />
        </div>
        <div className="lg:col-span-1 h-[400px]">
          <IamAuditPanel auditLogs={d.auditLogs} />
        </div>
      </div>
    </div>
  );
