'use client';
import React, { useEffect } from 'react';
import { useUIStore } from '@/store/ui.store';
import { Breadcrumbs } from '@/components/ui/Breadcrumbs';
import { Skeleton } from '@/components/ui/Spinner';
import { cn } from '@/utils/cn';
import { TurnSessionsPanel } from '../components/TurnSessionsPanel';
import { TurnCredentialsPanel } from '../components/TurnCredentialsPanel';
import { RelayUsageLogsPanel } from '../components/RelayUsageLogsPanel';
import { useCoturn } from '../hooks/useCoturn';
import { Radio, Wifi, Activity, Shield, Users, Gauge } from 'lucide-react';

interface CoturnKPI {
  id: string; title: string; value: string | number;
  status: 'normal' | 'warning' | 'critical' | 'success'; icon: React.ElementType; subtitle?: string;
}

function CoturnKPICard({ kpi }: { kpi: CoturnKPI }) {
  const statusColors: Record<string, string> = {
    success: 'border-success/20 hover:border-success/40', normal: 'border-white/[0.06] hover:border-white/[0.12]',
    warning: 'border-warning/20 hover:border-warning/40', critical: 'border-emergency/20 hover:border-emergency/40 bg-emergency/[0.02]',
  };
  const valueColors: Record<string, string> = {
    success: 'text-success-light', normal: 'text-white', warning: 'text-warning-light', critical: 'text-emergency-light',
  };
  const Icon = kpi.icon;
  return (
    <div className={cn('group relative overflow-hidden rounded-xl border bg-surface-light p-4 transition-all duration-300 shadow-glass-sm flex flex-col justify-between hover:-translate-y-0.5 hover:shadow-card-hover', statusColors[kpi.status])}>
      <div className="flex items-start justify-between">
        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{kpi.title}</p>
        <div className="p-2 rounded-lg bg-emerald-500/15"><Icon className="w-4 h-4 text-emerald-400" /></div>
      </div>
      <p className={cn('text-2xl font-black mt-2 font-mono', valueColors[kpi.status])}>{typeof kpi.value === 'number' ? kpi.value.toLocaleString() : kpi.value}</p>
      {kpi.subtitle && <p className="text-[10px] text-gray-500 mt-1">{kpi.subtitle}</p>}
    </div>
  );
}

export const CoturnDashboard: React.FC = () => {
  const setPageMeta = useUIStore((s) => s.setPageMeta);
  const { useSessions } = useCoturn();
  const sessionsQuery = useSessions();

  useEffect(() => {
    setPageMeta('CoTURN Relay', 'TURN/STUN relay server for telehealth and WebRTC media traversal');
  }, [setPageMeta]);

  if (sessionsQuery.isLoading) {
    return (
      <div className="space-y-6 animate-fade-in max-w-[1600px]">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[...Array(4)].map((_, i) => <Skeleton key={i} className="h-28 w-full rounded-xl" />)}
        </div>
      </div>
    );
  }

  const kpis: CoturnKPI[] = [
    { id: 'sessions', title: 'Active Relay Sessions', value: 47, status: 'success', icon: Radio, subtitle: 'Telehealth + video calls' },
    { id: 'bandwidth', title: 'Bandwidth Used', value: '2.8 Gbps', status: 'normal', icon: Wifi, subtitle: 'Current throughput' },
    { id: 'peers', title: 'Connected Peers', value: 94, status: 'success', icon: Users, subtitle: 'Doctor ↔ Patient pairs' },
    { id: 'latency', title: 'Relay Latency', value: '12ms', status: 'success', icon: Activity, subtitle: 'Median round-trip' },
    { id: 'auth', title: 'Auth Success Rate', value: '99.8%', status: 'success', icon: Shield, subtitle: 'TURN credentials' },
    { id: 'utilization', title: 'Server Load', value: '34%', status: 'success', icon: Gauge, subtitle: '3 TURN servers' },
  ];

  return (
    <div className="space-y-5 animate-fade-in max-w-[1600px]">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <Breadcrumbs items={[{ label: 'Infrastructure' }, { label: 'CoTURN Relay' }]} />
        <div className="flex items-center gap-2 text-[11px] font-bold text-emerald-300 bg-emerald-500/10 px-4 py-2 rounded-lg border border-emerald-500/25">
          <Radio className="w-3.5 h-3.5" />
          MEDIA RELAY SERVER
        </div>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        {kpis.map(kpi => <CoturnKPICard key={kpi.id} kpi={kpi} />)}
      </div>

      <div className="grid grid-cols-1 xl:grid-cols-12 gap-5">
        <div className="xl:col-span-7 h-[450px]">
          <TurnSessionsPanel />
        </div>
        <div className="xl:col-span-5 h-[450px]">
          <TurnCredentialsPanel />
        </div>
      </div>

      <div className="grid grid-cols-1 gap-5">
        <RelayUsageLogsPanel />
      </div>
    </div>
  );
};
