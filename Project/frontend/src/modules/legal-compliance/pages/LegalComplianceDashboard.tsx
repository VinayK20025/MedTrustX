...
import { useLegalComplianceDashboard } from '../hooks/useLegalComplianceAnalytics';
import type { ComplianceKPI } from '../types/legal-compliance.types';
import { Scale, ShieldAlert, FileWarning } from 'lucide-react';

function LcKPICard({ kpi }: { kpi: ComplianceKPI }) {
...
export function LegalComplianceDashboard() {
  ...
  const { data, isLoading } = useLegalComplianceDashboard({});

  useEffect(() => {
    setPageMeta('Legal Compliance Officer', 'Statutory compliance tracking, risk mitigation, and license management');
  }, [setPageMeta]);
  ...
  const criticalViolation = d.violations.find(v => v.severity === 'Critical');
  ...
  return (
    <div className="space-y-4 animate-fade-in max-w-[1600px] mx-auto">
      ...
      {criticalViolation && (
        <div className="bg-emergency/20 border border-emergency/40 rounded-xl px-5 py-3 flex items-center gap-3 animate-pulse">
          ...
          <div className="flex-1">
            <span className="text-[12px] font-black text-emergency-light uppercase tracking-widest">CRITICAL STATUTORY VIOLATION</span>
            <p className="text-[11px] text-red-300 mt-0.5">[{criticalViolation.department}] {criticalViolation.issue} Risk: {criticalViolation.penaltyRisk}</p>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <Breadcrumbs items={[{ label: 'Legal & Compliance' }, { label: 'Compliance Officer Command' }]} />
      </div>

      {/* KPIs */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {d.kpis.map(k => <LcKPICard key={k.id} kpi={k} />)}
      </div>

      {/* Main Layout */}
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-5 h-[calc(100vh-290px)] min-h-[600px]">
        <div className="xl:col-span-8 h-full">
          <ComplianceWorkspace regulations={d.regulations} checklists={d.checklists} licenses={d.licenses} />
        </div>
        <div className="xl:col-span-4 h-full">
          <ComplianceRiskPanel licenses={d.licenses} violations={d.violations} />
        </div>
      </div>
    </div>
  );
