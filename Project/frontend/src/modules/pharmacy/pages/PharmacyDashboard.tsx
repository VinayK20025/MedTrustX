...
import { PharmacyDispensePanel } from '../components/PharmacyDispensePanel';
...
import { usePharmacyDashboard } from '../hooks/usePharmacyAnalytics';
import type { DispensingFilters } from '../services/pharmacy.api';
import type { PharmacyKPI } from '../types/pharmacy.types';
...
export function PharmacyDashboard() {
  ...
  const [filters] = useState<DispensingFilters>({});
  const { data, isLoading } = usePharmacyDashboard(filters);

  useEffect(() => {
    setPageMeta('Pharmacist (Dispensing)', 'High-speed prescription verification, barcode scanning, and fulfillment');
  }, [setPageMeta]);
  ...
  return (
    <div className="space-y-5 animate-fade-in max-w-[1600px]">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <Breadcrumbs items={[{ label: 'Pharmacy' }, { label: 'Dispensing Operations' }]} />
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-5">
        ...
        <div className="xl:col-span-4 h-[600px]">
          <PharmacyQueue queue={d.queue || []} />
        </div>
        
        {/* Active Dispensing Terminal */}
        <div className="xl:col-span-5 h-[600px]">
          <PharmacyDispensePanel activePrescription={d.activePrescription} />
        </div>

        {/* Support Tools */}
        <div className="xl:col-span-3 h-[600px] flex flex-col gap-5">
           <div className="flex-1">
             <PharmacyInventoryPanel inventory={d.inventorySearch || []} />
           </div>
           <div className="flex-1">
             <PharmacyAlertPanel alerts={d.alerts || []} />
           </div>
        </div>
      </div>
    </div>
