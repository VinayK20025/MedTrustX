...
import { GuidedTaskPanel } from '../components/GuidedTaskPanel';
import { JRLearningPanel } from '../components/JRLearningPanel';
import { JRAlertsPanel } from '../components/JRAlertsPanel';
import { useJRDashboard } from '../hooks/useJRAnalytics';
...
export function JRDashboard() {
  const setPageMeta = useUIStore((s) => s.setPageMeta);
  const { data, isLoading } = useJRDashboard({});

  useEffect(() => { setPageMeta('Guided Clinical Workspace', 'Step-by-step execution and learning'); }, [setPageMeta]);
  ...
  return (
    <div className="space-y-5 animate-fade-in max-w-[1600px]">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <Breadcrumbs items={[{ label: 'Junior Resident' }, { label: 'Dashboard' }]} />
        <div className="flex items-center gap-2 text-sm font-mono text-teal-300 bg-teal-500/10 px-3 py-1.5 rounded-lg border border-teal-500/20">
          <GraduationCap className="w-4 h-4" /> SUPERVISOR: DR. SHARMA
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-5 h-auto xl:h-[650px]">
        {/* Left: Guided Execution (Core) */}
        <div className="xl:col-span-8 h-[600px] xl:h-full">
          <GuidedTaskPanel tasks={d.tasks} />
        </div>
        
        {/* Right: Learning & Alerts */}
        <div className="xl:col-span-4 flex flex-col gap-5 h-[600px] xl:h-full">
          <div className="flex-1">
            <JRLearningPanel protocols={d.protocols} />
          </div>
          <div className="h-[280px]">
            <JRAlertsPanel alerts={d.alerts} />
          </div>
        </div>
      </div>
    </div>
