...
import { CompliancePanel } from '@/modules/ciso/components/CompliancePanel';
...
export default function CisoCompliancePage() {
  ...
  return (
    <RoleGuard roles={['ciso', 'enterprise-security', 'security-root', 'super_admin']} requireAll={false}>
      ...
      <Breadcrumbs items={[{ label: 'CISO' }, { label: 'Compliance' }]} />
      
      {isLoading ? (
        <Skeleton className="h-[600px] w-full rounded-xl" />
      ) : data?.data ? (
        <div className="h-[700px]">
          <CompliancePanel metrics={data.data.compliance} />
        </div>
      ) : (
