...
import { SecurityIncidentPanel } from '@/modules/ciso/components/SecurityIncidentPanel';
...
export default function CisoIncidentsPage() {
  ...
  return (
    <RoleGuard roles={['ciso', 'enterprise-security', 'security-root', 'super_admin']} requireAll={false}>
      ...
      <Breadcrumbs items={[{ label: 'CISO' }, { label: 'Incidents' }]} />
      
      {isLoading ? (
        <Skeleton className="h-[600px] w-full rounded-xl" />
      ) : data?.data ? (
        <div className="h-[700px]">
          <SecurityIncidentPanel incidents={data.data.incidents} />
        </div>
      ) : (
