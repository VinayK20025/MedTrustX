...
import { ThreatPanel } from '@/modules/ciso/components/ThreatPanel';
...
export default function CisoThreatsPage() {
  ...
  return (
    <RoleGuard roles={['ciso', 'enterprise-security', 'security-root', 'super_admin']} requireAll={false}>
      ...
      <Breadcrumbs items={[{ label: 'CISO' }, { label: 'Threats' }]} />
      
      {isLoading ? (
        <Skeleton className="h-[600px] w-full rounded-xl" />
      ) : data?.data ? (
        <div className="h-[700px]">
          <ThreatPanel threats={data.data.threats} />
        </div>
      ) : (
