...
export default function NetworkIncidentsPage() {
  return (
    <div className="space-y-5 animate-fade-in max-w-[1200px]">
      <Breadcrumbs items={[{ label: 'IT Administration' }, { label: 'Network Incidents' }]} />
      <Card className="border-white/[0.06] shadow-glass bg-surface-light min-h-[400px] flex items-center justify-center">
        <CardBody className="text-gray-500 text-center">
          <p className="text-lg">Connectivity Logs & Support Tickets</p>
          <p className="text-sm">Resolve unreachable devices, high-latency nodes, and AP failures.</p>
        </CardBody>
      </Card>
    </div>
  );
