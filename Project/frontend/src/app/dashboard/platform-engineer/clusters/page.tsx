...
import { PlatformClusterPanel } from '@/modules/platform-engineer/components/PlatformClusterPanel';
...
export default function PlatformEngineerClustersPage() {
  const [filters] = useState<PlatformFilters>({});
  const { data, isLoading } = usePlatformDashboard(filters);

  return (
    <div className="space-y-5 animate-fade-in max-w-[1600px]">
      <Breadcrumbs items={[{ label: 'Platform Engineer' }, { label: 'Clusters' }]} />
      
      {isLoading ? (
        <Skeleton className="h-[600px] w-full rounded-xl" />
      ) : data?.data ? (
        <div className="h-[700px]">
          <PlatformClusterPanel clusters={data.data.clusters} nodes={data.data.nodes} />
        </div>
      ) : (
