...
import { PlatformAlertPanel } from '@/modules/platform-engineer/components/PlatformAlertPanel';
...
export default function PlatformEngineerAlertsPage() {
  const [filters] = useState<PlatformFilters>({});
  const { data, isLoading } = usePlatformDashboard(filters);

  return (
    <div className="space-y-5 animate-fade-in max-w-[1600px]">
      <Breadcrumbs items={[{ label: 'Platform Engineer' }, { label: 'Alerts' }]} />
      
      {isLoading ? (
        <Skeleton className="h-[600px] w-full rounded-xl" />
      ) : data?.data ? (
        <div className="h-[700px]">
          <PlatformAlertPanel alerts={data.data.alerts} />
        </div>
      ) : (
