...
import { HrCredentialPanel, useHrDashboard } from '@/modules/hr';
  ...
  return (
    <RoleGuard roles={HR_ROLES}>
      <div className="space-y-5 animate-fade-in max-w-[800px]">
        <Breadcrumbs items={[{ label: 'HR' }, { label: 'License & Credential Tracker' }]} />
        <div className="h-[700px]"><HrCredentialPanel alerts={data?.data?.credentialAlerts ?? []} /></div>
      </div>
    </RoleGuard>
  );
}
