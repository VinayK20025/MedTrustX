"""
MedTrustX Notification Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter(
    "notif_service_requests_total",
    "Total HTTP requests", ["method", "endpoint", "status"],
)
REQUEST_LATENCY = Histogram(
    "notif_service_request_duration_seconds",
...
app = FastAPI(
    title="MedTrustX Notification Service",
    description="Central communication hub for multi-channel message delivery.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if any(p in path for p in ["/notifications", "/templates", "/users"]):
        parts = path.split("/")
        normalized = []
        for part in parts:
            try:
