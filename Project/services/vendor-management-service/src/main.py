"""
MedTrustX Vendor Management Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter("vm_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("vm_request_duration_seconds", "Request latency in seconds", ["method", "endpoint"])
...
app = FastAPI(
    title="MedTrustX Vendor Management Service",
    description="Vendor onboarding, contracts, SLAs, and performance.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    for prefix in ("/vendors/", "/contracts/", "/slas/"):
        if prefix in path:
            parts = path.split("/")
            normalized = []
            for i, part in enumerate(parts):
                if i > 0 and parts[i - 1] in ("vendors", "contracts", "slas"):
