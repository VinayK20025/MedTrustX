"""
MedTrustX Compliance Governance Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter("compliance_gov_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("compliance_gov_request_duration_seconds", "Request latency in seconds", ["method", "endpoint"])
...
app = FastAPI(
    title="MedTrustX Compliance Governance Service",
    description="Compliance policies, controls, violations, and evidence tracking.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    for prefix in ("/compliance/policies/", "/compliance/controls/", "/compliance/violations", "/compliance/evidence", "/compliance/reports"):
        if prefix in path:
            parts = path.split("/")
            normalized = []
            for i, part in enumerate(parts):
                if i > 0 and parts[i - 1] in ("policies", "controls"):
...
from src.routes.compliance_governance import router as compliance_governance_router
