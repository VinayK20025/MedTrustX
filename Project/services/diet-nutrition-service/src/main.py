"""
MedTrustX Diet & Nutrition Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter("diet_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("diet_request_duration_seconds", "Request latency in seconds", ["method", "endpoint"])
...
app = FastAPI(
    title="MedTrustX Diet & Nutrition Service",
    description="Patient dietary plans, nutrition profiles, and meal orders.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    for prefix in ("/diet-plans/", "/nutrition-profiles/", "/meal-orders/"):
        if prefix in path:
            parts = path.split("/")
            normalized = []
            for i, part in enumerate(parts):
                if i > 0 and parts[i - 1] in ("diet-plans", "nutrition-profiles", "meal-orders"):
...
from src.routes.diet import router as diet_router
