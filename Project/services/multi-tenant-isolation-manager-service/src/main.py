"""
MedTrustX Multi-Tenant Isolation Manager Service — Application Entrypoint
"""
import time, logging
...
REQUEST_COUNT = Counter("isolation_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("isolation_request_duration_seconds", "Request latency", ["method", "endpoint"])
...
app = FastAPI(title="MedTrustX Multi-Tenant Isolation Manager Service", description="Tier-0 boundary enforcement layer.", version=settings.SERVICE_VERSION, docs_url="/docs", redoc_url="/redoc", openapi_url="/openapi.json", lifespan=lifespan)
...
async def metrics_middleware(request: Request, call_next):
    ...
    if path.startswith("/tenants/") and len(path.split("/")) > 2:
        parts = path.split("/")
        parts[2] = "{id}"
        path = "/".join(parts)
...
from src.routes.isolation import router as iso_router
