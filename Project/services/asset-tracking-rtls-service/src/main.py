"""
MedTrustX Asset Tracking RTLS Service — Application Entrypoint
"""
import time, logging
...
REQUEST_COUNT = Counter("rtls_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("rtls_request_duration_seconds", "Request latency", ["method", "endpoint"])
...
app = FastAPI(title="MedTrustX Asset Tracking RTLS Service", description="Tier-0 operational visibility layer.", version=settings.SERVICE_VERSION, docs_url="/docs", redoc_url="/redoc", openapi_url="/openapi.json", lifespan=lifespan)
...
async def metrics_middleware(request: Request, call_next):
    ...
    if path.startswith("/assets/") and len(path.split("/")) > 2:
        parts = path.split("/")
        parts[2] = "{id}"
        path = "/".join(parts)
...
from src.routes.rtls import router as rtls_router
app.include_router(health_router, tags=["Health"])
app.include_router(rtls_router)
