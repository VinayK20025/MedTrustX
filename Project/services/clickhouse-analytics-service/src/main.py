"""
MedTrustX ClickHouse Analytics Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter("clickhouse_analytics_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("clickhouse_analytics_request_duration_seconds", "Request latency in seconds", ["method", "endpoint"])
...
app = FastAPI(
    title="MedTrustX ClickHouse Analytics Service",
    description="High-performance OLAP data ingestion and querying API.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if path.startswith("/analytics/") and len(path.split("/")) > 2:
        parts = path.split("/")
        if parts[2] not in ["ingest", "events", "metrics"]:
            parts[2] = "{id}"
        path = "/".join(parts)
