"""
MedTrustX Alert Correlation Engine Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter("alert_correlation_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("alert_correlation_request_duration_seconds", "Request latency in seconds", ["method", "endpoint"])
...
app = FastAPI(
    title="MedTrustX Alert Correlation Engine Service",
    description="Tier-0 intelligent alert aggregation, correlation, and deduplication.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if "/alerts/" in path or "/incidents/" in path:
        parts = path.split("/")
        if len(parts) > 2:
            parts[2] = "{id}"
            path = "/".join(parts)
...
from src.routes.correlation import router as correlation_router
