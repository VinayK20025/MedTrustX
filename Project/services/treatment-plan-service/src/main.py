"""
MedTrustX Treatment Plan Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter(
    "treatment_service_requests_total",
    "Total HTTP requests",
    ["method", "endpoint", "status"],
)
REQUEST_LATENCY = Histogram(
    "treatment_service_request_duration_seconds",
...
app = FastAPI(
    title="MedTrustX Treatment Plan Service",
    description="Structured medical planning, intent modeling, and adherence tracking.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if "/treatment-plans" in path or "/patient" in path:
        parts = path.split("/")
        normalized = []
        for part in parts:
            try:
...
from src.routes.adherence import router as adherence_router

app.include_router(health_router, tags=["Health"])
app.include_router(plans_router, prefix="/api/v1", tags=["Plans"])
app.include_router(items_router, prefix="/api/v1", tags=["Items"])
app.include_router(adherence_router, prefix="/api/v1", tags=["Adherence"])
