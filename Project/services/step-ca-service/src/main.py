"""
MedTrustX Step-CA Shim Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter(
    "step_ca_requests_total",
    "Total HTTP requests", ["method", "endpoint", "status"],
)
REQUEST_LATENCY = Histogram(
    "step_ca_request_duration_seconds",
...
app = FastAPI(
    title="MedTrustX Step-CA Shim Service",
    description="Internal Certificate Authority for mTLS.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if "/1.0/certs/" in path:
        parts = path.split("/")
        normalized = []
        for i, part in enumerate(parts):
            if i > 0 and parts[i-1] == "certs":
                normalized.append("{cert_id}")
            else:
...
from src.routes.ca import router as ca_router
