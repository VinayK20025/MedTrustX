"""
MedTrustX Care Coordination Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter(
    "coordination_service_requests_total",
    "Total HTTP requests",
    ["method", "endpoint", "status"],
)
REQUEST_LATENCY = Histogram(
    "coordination_service_request_duration_seconds",
...
app = FastAPI(
    title="MedTrustX Care Coordination Service",
    description="Cross-department orchestration of patient care journeys.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if "/care-plans" in path or "/care-tasks" in path or "/care-workflows" in path or "/patient" in path:
        parts = path.split("/")
        normalized = []
        for part in parts:
            try:
...
app.include_router(plans_router, prefix="/api/v1", tags=["Care Plans"])
app.include_router(tasks_router, prefix="/api/v1", tags=["Care Tasks"])
