"""
MedTrustX Access Control Service — Application Entrypoint
"""
import time
...
REQUEST_LATENCY = Histogram(
    "access_request_duration_seconds",
...
app = FastAPI(
    title="MedTrustX Access Control Service",
    description="Central authorization decision engine.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if any(p in path for p in ["/access", "/policy-bindings", "/attributes"]):
        parts = path.split("/")
        normalized = []
        for part in parts:
            try:
