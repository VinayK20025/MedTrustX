"""
MedTrustX Performance Management Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter("performance_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("performance_request_duration_seconds", "Request latency in seconds", ["method", "endpoint"])
...
app = FastAPI(
    title="MedTrustX Performance Management Service",
    description="Measurement, evaluation, and optimization of performance.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    for prefix in ("/kpis/", "/performance-records", "/scorecards/", "/evaluations", "/benchmarks"):
        if prefix in path:
            parts = path.split("/")
            normalized = []
            for i, part in enumerate(parts):
                if i > 0 and parts[i - 1] in ("kpis", "scorecards"):
