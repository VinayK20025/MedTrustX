"""
MedTrustX Fleet Management Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter("fleet_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("fleet_request_duration_seconds", "Request latency in seconds", ["method", "endpoint"])
...
app = FastAPI(
    title="MedTrustX Fleet Management Service",
    description="Hospital fleets, ambulances, routing, and logistics tracking.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    for prefix in ("/vehicles/", "/drivers/", "/trips/", "/assignments/"):
        if prefix in path:
            parts = path.split("/")
            normalized = []
            for i, part in enumerate(parts):
                if i > 0 and parts[i - 1] in ("vehicles", "drivers", "trips", "assignments"):
