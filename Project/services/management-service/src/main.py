"""
MedTrustX Management Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter(
    "mgmt_service_requests_total",
    "Total HTTP requests", ["method", "endpoint", "status"],
)
REQUEST_LATENCY = Histogram(
    "mgmt_service_request_duration_seconds",
...
app = FastAPI(
    title="MedTrustX Management Service",
    description="Administrative control plane for platform configuration and feature toggling.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if any(p in path for p in ["/configs", "/feature-flags", "/tenant-settings", "/service-configs", "/config-audit-logs"]):
        parts = path.split("/")
        normalized = []
        for part in parts:
            try:
...
from src.routes.flags import router as flags_router
