"""
MedTrustX Workflow Engine — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter("workflow_engine_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("workflow_engine_request_duration_seconds", "Request latency in seconds", ["method", "endpoint"])
...
app = FastAPI(
    title="MedTrustX Workflow Engine",
    description="Cross-service business process orchestration, tasks, and transitions.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    for prefix in ("/workflows/", "/workflow-definitions", "/workflow-instances/", "/tasks/"):
        if prefix in path:
            parts = path.split("/")
            normalized = []
            for i, part in enumerate(parts):
                if i > 0 and parts[i - 1] in ("workflows", "workflow-instances", "tasks"):
