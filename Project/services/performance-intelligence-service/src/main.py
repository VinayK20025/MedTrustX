"""
MedTrustX Performance Intelligence Service — Application Entrypoint
"""
import time, logging
...
REQUEST_COUNT = Counter("perfintel_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("perfintel_request_duration_seconds", "Request latency", ["method", "endpoint"])
...
app = FastAPI(title="MedTrustX Performance Intelligence Service", description="Tier-0 cross-domain performance analysis and optimization.", version=settings.SERVICE_VERSION, docs_url="/docs", redoc_url="/redoc", openapi_url="/openapi.json", lifespan=lifespan)
...
async def metrics_middleware(request: Request, call_next):
    ...
    if path.startswith("/performance/") and len(path.split("/")) > 2 and path.split("/")[2] != "evaluate":
        parts = path.split("/")
...
from src.routes.performance import router as perf_router
