"""
MedTrustX Threat Detection Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter(
    "threat_detection_requests_total",
    "Total HTTP requests",
    ["method", "endpoint", "status"],
)
REQUEST_LATENCY = Histogram(
    "threat_detection_request_duration_seconds",
...
app = FastAPI(
    title="MedTrustX Threat Detection Service",
    description="AI-driven behavioural analytics and anomaly detection engine.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if "/risk-score/" in path:
        parts = path.split("/")
        normalized = []
        for i, part in enumerate(parts):
            if i > 0 and parts[i - 1] == "risk-score":
...
from src.routes.threat import router as threat_router
