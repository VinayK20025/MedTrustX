"""
MedTrustX Pharmacy Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter(
    "pharmacy_service_requests_total",
    "Total HTTP requests",
    ["method", "endpoint", "status"],
)
REQUEST_LATENCY = Histogram(
    "pharmacy_service_request_duration_seconds",
...
app = FastAPI(
    title="MedTrustX Pharmacy Service",
    description="Governs prescription, dispensing, and administration workflows.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if "/prescriptions/" in path or "/patients/" in path:
        parts = path.split("/")
        normalized = []
        for part in parts:
            try:
...
from src.routes.dispenses import router as dispenses_router
from src.routes.administrations import router as administrations_router
...
app.include_router(dispenses_router, prefix="/api/v1", tags=["Dispenses"])
app.include_router(administrations_router, prefix="/api/v1", tags=["Administrations"])
app.include_router(history_router, prefix="/api/v1", tags=["Medication History"])
