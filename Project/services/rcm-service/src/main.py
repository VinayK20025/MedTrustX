"""
MedTrustX RCM Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter(
    "rcm_service_requests_total",
    "Total HTTP requests",
    ["method", "endpoint", "status"],
)
REQUEST_LATENCY = Histogram(
    "rcm_service_request_duration_seconds",
...
app = FastAPI(
    title="MedTrustX RCM Service",
    description="Revenue Cycle Management for insurance claims and reimbursements.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if "/claims" in path or "/reimbursements" in path:
        parts = path.split("/")
        normalized = []
        for part in parts:
            try:
...
from src.routes.reimbursements import router as reimbursements_router
