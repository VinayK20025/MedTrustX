"""
MedTrustX API Composition Gateway Service — Application Entrypoint
"""
import time, logging
...
REQUEST_COUNT = Counter("api_composition_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("api_composition_request_duration_seconds", "Request latency", ["method", "endpoint"])
...
app = FastAPI(title="MedTrustX API Composition Gateway Service", description="Tier-0 unified API access layer.", version=settings.SERVICE_VERSION, docs_url="/docs", redoc_url="/redoc", openapi_url="/openapi.json", lifespan=lifespan)
...
async def metrics_middleware(request: Request, call_next):
    ...
    if path.startswith("/compositions/") and len(path.split("/")) > 2:
...
from src.routes.composition import router as comp_router
