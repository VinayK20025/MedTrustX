"""
MedTrustX Clinical Research Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter(
    "clinical_research_requests_total",
    "Total HTTP requests",
    ["method", "endpoint", "status"],
)
REQUEST_LATENCY = Histogram(
    "clinical_research_request_duration_seconds",
...
app = FastAPI(
    title="MedTrustX Clinical Research Service",
    description="Clinical trials, cohort management, and research data governance.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    for prefix in ("/studies/", "/cohorts/"):
        if prefix in path:
            parts = path.split("/")
            normalized = []
            for i, part in enumerate(parts):
                if i > 0 and parts[i - 1] in ("studies", "cohorts"):
