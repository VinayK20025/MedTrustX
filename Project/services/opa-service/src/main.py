"""
MedTrustX OPA Shim Service — Application Entrypoint
"""
import time
...
app = FastAPI(
    title="MedTrustX OPA Shim Service",
    description="Policy Decision Point Shim.",
...
# NOTE: No ZTAMiddleware applied here since this service IS the authorization engine. 
# Applying ZTA to OPA would result in an infinite loop.
...
async def metrics_middleware(request: Request, call_next):
    ...
    if "/v1/data/" in path or "/v1/policies/" in path:
        parts = path.split("/")
        normalized = []
        for i, part in enumerate(parts):
            if i > 0 and parts[i-1] in ("data", "policies"):
