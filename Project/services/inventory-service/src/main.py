"""
MedTrustX Inventory Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter(
    "inventory_service_requests_total",
    "Total HTTP requests",
    ["method", "endpoint", "status"],
)
REQUEST_LATENCY = Histogram(
    "inventory_service_request_duration_seconds",
...
app = FastAPI(
    title="MedTrustX Inventory Service",
    description="Hospital stock lifecycle and supply chain tracking.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if "/items" in path or "/batches" in path or "/movements" in path or "/reservations" in path:
        parts = path.split("/")
        normalized = []
        for part in parts:
            try:
