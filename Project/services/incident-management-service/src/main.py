"""
MedTrustX Incident Management Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter("inc_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("inc_request_duration_seconds", "Request latency in seconds", ["method", "endpoint"])
...
app = FastAPI(
    title="MedTrustX Incident Management Service",
    description="Incident detection, triage, RCA, and playbooks.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    for prefix in ("/incidents/", "/playbooks"):
        if prefix in path:
            parts = path.split("/")
            normalized = []
            for i, part in enumerate(parts):
                if i > 0 and parts[i - 1] in ("incidents",):
