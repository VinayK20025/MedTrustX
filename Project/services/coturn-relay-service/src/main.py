"""
MedTrustX Coturn Relay Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter("coturn_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("coturn_request_duration_seconds", "Request latency in seconds", ["method", "endpoint"])
...
app = FastAPI(
    title="MedTrustX Coturn Relay Service",
    description="TURN/STUN media relay provisioning and tracking.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if path.startswith("/turn/") and len(path.split("/")) > 3:
        parts = path.split("/")
        parts[3] = "{id}"
        path = "/".join(parts)
...
from src.routes.coturn import router as coturn_router

app.include_router(health_router, tags=["Health"])
app.include_router(coturn_router)
