"""
MedTrustX Prometheus Monitoring Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter("prometheus_monitoring_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("prometheus_monitoring_request_duration_seconds", "Request latency in seconds", ["method", "endpoint"])
...
app = FastAPI(
    title="MedTrustX Prometheus Monitoring Service",
    description="Tier-0 observability layer tracking PromQL metrics and alerts.",
...
from src.routes.prometheus import router as prometheus_router
