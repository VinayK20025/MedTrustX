"""
MedTrustX Nursing Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter(
    "nursing_service_requests_total",
    "Total HTTP requests",
    ["method", "endpoint", "status"],
)
REQUEST_LATENCY = Histogram(
    "nursing_service_request_duration_seconds",
...
app = FastAPI(
    title="MedTrustX Nursing Service",
    description="Coordinates and tracks nursing workflows, shift handovers, and bedside observations.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if "/nursing/" in path or "/patients/" in path:
        parts = path.split("/")
        normalized = []
        for part in parts:
            try:
...
from src.routes.shifts import router as shifts_router

app.include_router(health_router, tags=["Health"])
app.include_router(tasks_router, prefix="/api/v1", tags=["Nursing Tasks"])
app.include_router(vitals_router, prefix="/api/v1", tags=["Vitals"])
app.include_router(notes_router, prefix="/api/v1", tags=["Nursing Notes"])
app.include_router(shifts_router, prefix="/api/v1", tags=["Shift Handovers"])
