"""
MedTrustX Transplant Coordination Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter("tc_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("tc_request_duration_seconds", "Request latency in seconds", ["method", "endpoint"])
...
app = FastAPI(
    title="MedTrustX Transplant Coordination Service",
    description="Organ transplant workflow and matching orchestration.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    for prefix in ("/donors/", "/recipients/", "/waitlists", "/matches/", "/transplant-events"):
        if prefix in path and path not in ("/waitlists", "/transplant-events"):
            parts = path.split("/")
            normalized = []
            for i, part in enumerate(parts):
                if i > 0 and parts[i - 1] in ("donors", "recipients", "matches"):
...
from src.routes.transplant import router as transplant_router

app.include_router(health_router, tags=["Health"])
app.include_router(transplant_router)
