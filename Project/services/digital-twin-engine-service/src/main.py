"""
MedTrustX Digital Twin Engine Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter("digital_twin_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("digital_twin_request_duration_seconds", "Request latency in seconds", ["method", "endpoint"])
...
app = FastAPI(
    title="MedTrustX Digital Twin Engine Service",
    description="Tier-0 real-time digital twin state modelling and simulation engine.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if path.startswith("/twins/") and len(path.split("/")) > 2:
        parts = path.split("/")
        parts[2] = "{id}"
        path = "/".join(parts)
...
from src.routes.twin import router as twin_router
