"""
MedTrustX IAM Service — Application Entrypoint
"""
import time
...
app = FastAPI(
    title="MedTrustX IAM Service",
    description="Identity, authentication, and trust propagation foundation.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if any(p in path for p in ["/auth/", "/users/", "/roles/", "/sessions/"]):
        parts = path.split("/")
        normalized = []
        for part in parts:
            try:
...
app.include_router(auth_router, prefix="/api/v1/iam")
app.include_router(users_router, prefix="/api/v1/iam")
app.include_router(roles_router, prefix="/api/v1/iam")
app.include_router(sessions_router, prefix="/api/v1/iam")
