...
export class ZtaGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    let resource = 'mm_resource';
    if (request.path.includes('/mortuary/records')) resource = 'mm_records';
    else if (request.path.includes('/storage-units')) resource = 'mm_storage';
    else if (request.path.includes('/allocations')) resource = 'mm_allocations';
    else if (request.path.includes('/custody-logs')) resource = 'mm_custody';
    else if (request.path.includes('/releases')) resource = 'mm_releases';
