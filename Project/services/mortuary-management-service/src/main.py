"""
MedTrustX Mortuary Management Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter("mm_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("mm_request_duration_seconds", "Request latency in seconds", ["method", "endpoint"])
...
app = FastAPI(
    title="MedTrustX Mortuary Management Service",
    description="Post-death workflow and body handling.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    for prefix in ("/mortuary/records/", "/storage-units/", "/allocations/", "/custody-logs/", "/releases/"):
        if prefix in path:
            parts = path.split("/")
            normalized = []
            for i, part in enumerate(parts):
                if i > 0 and parts[i - 1] in ("records", "storage-units", "allocations", "custody-logs", "releases"):
...
from src.routes.mortuary import router as mortuary_router
