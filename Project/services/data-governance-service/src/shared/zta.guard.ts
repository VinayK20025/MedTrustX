...
export class ZtaGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    let resource = 'dg_resource';
    if (request.path.includes('/data-assets')) resource = 'dg_assets';
    else if (request.path.includes('/classifications')) resource = 'dg_classifications';
    else if (request.path.includes('/lineage')) resource = 'dg_lineage';
    else if (request.path.includes('/quality-rules')) resource = 'dg_quality_rules';
    else if (request.path.includes('/retention-policies')) resource = 'dg_retention_policies';
