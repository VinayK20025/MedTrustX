"""
MedTrustX Data Governance Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter("data_governance_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("data_governance_request_duration_seconds", "Request latency in seconds", ["method", "endpoint"])
...
app = FastAPI(
    title="MedTrustX Data Governance Service",
    description="Data catalog, classifications, lineage, and retention policies.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    for prefix in ("/data-assets/", "/classifications/", "/lineage/", "/quality-rules/", "/retention-policies/"):
        if prefix in path:
            parts = path.split("/")
            normalized = []
            for i, part in enumerate(parts):
                if i > 0 and parts[i - 1] in ("data-assets", "classifications", "lineage", "quality-rules", "retention-policies"):
...
from src.routes.data_governance import router as data_governance_router
