"""
MedTrustX Gitea Source Control Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter("gitea_source_control_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("gitea_source_control_request_duration_seconds", "Request latency in seconds", ["method", "endpoint"])
...
app = FastAPI(
    title="MedTrustX Gitea Source Control Service",
    description="Tier-1 Git repository management and collaboration platform.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if path.startswith("/repos/") and len(path.split("/")) > 2:
        parts = path.split("/")
        parts[2] = "{id}"
        path = "/".join(parts)
...
from src.routes.gitea import router as gitea_router
