"""
MedTrustX Emergency (ER) Service — Database Engine & Session Factory
...
# ── Session factory (no tenant scoping — raw) ──────────────────
...
async def get_session() -> AsyncGenerator[AsyncSession, None]:
    ...
    For tenant-scoped access use ``get_tenant_session`` instead.
    """
...
async def get_tenant_session(tenant_id: str) -> AsyncGenerator[AsyncSession, None]:
    ...
    to filter data transparently.

    Usage::

        async with get_tenant_session(tenant_id) as session:
            result = await session.execute(select(EmergencyCase))
    """
    async with AsyncSessionFactory() as session:
        try:
