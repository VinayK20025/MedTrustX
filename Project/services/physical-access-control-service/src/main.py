"""
MedTrustX Physical Access Control Service — Application Entrypoint
"""
import time, logging
...
REQUEST_COUNT = Counter("pac_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("pac_request_duration_seconds", "Request latency", ["method", "endpoint"])
...
app = FastAPI(title="MedTrustX Physical Access Control Service", description="Tier-0 facility access enforcement layer.", version=settings.SERVICE_VERSION, docs_url="/docs", redoc_url="/redoc", openapi_url="/openapi.json", lifespan=lifespan)
