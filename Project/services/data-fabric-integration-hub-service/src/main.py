"""
MedTrustX Data Fabric / Integration Hub Service — Application Entrypoint
"""
import time, logging
...
REQUEST_COUNT = Counter("data_fabric_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("data_fabric_request_duration_seconds", "Request latency", ["method", "endpoint"])
...
app = FastAPI(title="MedTrustX Data Fabric / Integration Hub Service", description="Tier-0 unified data integration and orchestration layer.", version=settings.SERVICE_VERSION, docs_url="/docs", redoc_url="/redoc", openapi_url="/openapi.json", lifespan=lifespan)
...
async def metrics_middleware(request: Request, call_next):
    ...
    if path.startswith("/pipelines/") and len(path.split("/")) > 2:
        parts = path.split("/"); parts[2] = "{id}"; path = "/".join(parts)
...
from src.routes.fabric import router as fabric_router
