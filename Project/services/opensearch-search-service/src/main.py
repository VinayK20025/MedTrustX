"""
MedTrustX OpenSearch Search Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter("opensearch_search_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("opensearch_search_request_duration_seconds", "Request latency in seconds", ["method", "endpoint"])
...
app = FastAPI(
    title="MedTrustX OpenSearch Search Service",
    description="Tier-1 full-text search and log analytics engine.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if path.startswith("/indexes/") and len(path.split("/")) > 2:
        parts = path.split("/")
        parts[2] = "{name}"
        path = "/".join(parts)
...
from src.routes.opensearch import router as opensearch_router
