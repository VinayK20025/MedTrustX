"""
MedTrustX Enterprise Risk Oversight Service — Application Entrypoint
"""
import time, logging
...
REQUEST_COUNT = Counter("riskoversight_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("riskoversight_request_duration_seconds", "Request latency", ["method", "endpoint"])
...
app = FastAPI(title="MedTrustX Enterprise Risk Oversight Service", description="Tier-0 centralized governance and enterprise risk management.", version=settings.SERVICE_VERSION, docs_url="/docs", redoc_url="/redoc", openapi_url="/openapi.json", lifespan=lifespan)
...
async def metrics_middleware(request: Request, call_next):
    ...
    if path.startswith("/risks/") and len(path.split("/")) > 2:
        parts = path.split("/")
        parts[2] = "{id}"
        path = "/".join(parts)
