"""
MedTrustX CDSS Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter(
    "cdss_service_requests_total",
    "Total HTTP requests",
    ["method", "endpoint", "status"],
)
REQUEST_LATENCY = Histogram(
    "cdss_service_request_duration_seconds",
...
app = FastAPI(
    title="MedTrustX Clinical Decision Support Service (CDSS)",
    description="Real-time clinical intelligence, rule evaluation, and alerts generation.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if "/rules" in path or "/alerts" in path or "/recommendations" in path or "/evaluate" in path:
        parts = path.split("/")
        normalized = []
        for part in parts:
            try:
...
app.include_router(evaluate_router, prefix="/api/v1", tags=["Engine"])
