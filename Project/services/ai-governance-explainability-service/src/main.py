"""
MedTrustX AI Governance & Explainability Service — Application Entrypoint
"""
import time, logging
...
REQUEST_COUNT = Counter("ai_governance_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("ai_governance_request_duration_seconds", "Request latency", ["method", "endpoint"])
...
app = FastAPI(title="MedTrustX AI Governance & Explainability Service", description="Tier-0 AI compliance and oversight layer.", version=settings.SERVICE_VERSION, docs_url="/docs", redoc_url="/redoc", openapi_url="/openapi.json", lifespan=lifespan)
...
async def metrics_middleware(request: Request, call_next):
    ...
    elif path.startswith("/decisions/") and len(path.split("/")) > 2:
        parts = path.split("/"); parts[2] = "{id}"; path = "/".join(parts)
...
from src.routes.governance import router as gov_router
