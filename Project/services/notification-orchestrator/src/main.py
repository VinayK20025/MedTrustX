"""
MedTrustX Notification Orchestrator Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter(
    "notif_orch_service_requests_total",
    "Total HTTP requests", ["method", "endpoint", "status"],
)
REQUEST_LATENCY = Histogram(
    "notif_orch_service_request_duration_seconds",
...
app = FastAPI(
    title="MedTrustX Notification Orchestrator Service",
    description="Intelligent orchestration engine for complex notification workflows.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if any(p in path for p in ["/notification-workflows", "/workflow-instances", "/escalation-rules", "/workflow-events"]):
        parts = path.split("/")
        normalized = []
        for part in parts:
            try:
...
# Include workflow-events route inside instances router for simplicity
from src.routes.instances import add_workflow_event
app.post("/api/v1/workflow-events", response_model=None, tags=["Workflow Events"])(add_workflow_event)
