"""
ZTA middleware — calls OPA for policy decision on admin API requests.

NOTE: The gateway itself is the *first* enforcement checkpoint; its own
admin API (service/route/consumer/plugin CRUD) must also be guarded.
Proxy traffic is not evaluated here — individual downstream services
own their own ZTA middleware.
"""
...
class ZTAMiddleware(BaseHTTPMiddleware):
    SKIP_PATHS = {"/health", "/ready", "/metrics", "/docs", "/redoc", "/openapi.json", "/status"}
        ...
        try:
            async with httpx.AsyncClient(timeout=2.0) as client:
                ...
                if not decision.get("allow", False):
                    ...
                    return JSONResponse(status_code=403, content={
                        "error": "access_denied", "reason": reason,
                    })
