"""
MedTrustX Kong Gateway Shim Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter(
    "kong_gateway_requests_total",
    "Total HTTP requests",
    ["method", "endpoint", "status"],
)
REQUEST_LATENCY = Histogram(
    "kong_gateway_request_duration_seconds",
...
app = FastAPI(
    title="MedTrustX Kong Gateway Shim Service",
    description="Edge API gateway — routing, JWT validation, rate limiting.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    # Normalise UUID segments
    for prefix in ("/services/", "/routes/", "/consumers/", "/plugins/"):
        if prefix in path:
            parts = path.split("/")
            normalized = []
            for i, part in enumerate(parts):
                if i > 0 and parts[i - 1] in ("services", "routes", "consumers", "plugins"):
