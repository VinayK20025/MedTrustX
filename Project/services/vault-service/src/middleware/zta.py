...
_RESOURCE_MAP = {
    "/v1/secret": "vault_secret",
    "/v1/database": "vault_database_cred",
    "/v1/transit": "vault_transit",
    "/v1/auth": "vault_auth",
}
...
class ZTAMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if "/v1/auth" in request.url.path:
            return await call_next(request)

        opa_input = {
            "input": {
                "role": request.headers.get("X-Vault-Token", request.headers.get("X-User-Role", "anonymous")),
