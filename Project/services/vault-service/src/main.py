"""
MedTrustX Vault Shim Service — Application Entrypoint
"""
import time
...
app = FastAPI(
    title="MedTrustX Vault Shim Service",
    description="Centralized Secrets and Cryptographic Control Plane.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if "/v1/" in path:
        ...
        for i, part in enumerate(parts):
            if i > 0 and parts[i-1] in ("data", "creds", "encrypt", "decrypt", "auth"):
                normalized.append("{dynamic_path}")
            else:
...
from src.routes.vault_routes import router as vault_router
