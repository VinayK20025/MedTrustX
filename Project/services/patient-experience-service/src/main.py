"""
MedTrustX Patient Experience Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter("experience_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("experience_request_duration_seconds", "Request latency in seconds", ["method", "endpoint"])
...
app = FastAPI(
    title="MedTrustX Patient Experience Service",
    description="Patient satisfaction, engagement, feedback, and service quality.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    for prefix in ("/feedback/", "/surveys/", "/survey-responses", "/complaints/", "/experience-scores/"):
        if prefix in path:
            parts = path.split("/")
            normalized = []
            for i, part in enumerate(parts):
                if i > 0 and parts[i - 1] in ("feedback", "surveys", "complaints", "experience-scores"):
...
from src.routes.experience import router as experience_router
