"""
MedTrustX Simulation & What-If Engine Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter("simulation_whatif_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("simulation_whatif_request_duration_seconds", "Request latency in seconds", ["method", "endpoint"])
...
app = FastAPI(
    title="MedTrustX Simulation & What-If Engine Service",
    description="Tier-0 predictive simulation and scenario analysis engine.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if path.startswith("/simulations/") and len(path.split("/")) > 2:
        parts = path.split("/")
        parts[2] = "{id}"
        path = "/".join(parts)
