"""
MedTrustX Redpanda Console Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter("redpanda_console_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("redpanda_console_request_duration_seconds", "Request latency in seconds", ["method", "endpoint"])
...
app = FastAPI(
    title="MedTrustX Redpanda Console Service",
    description="Tier-2 streaming management and monitoring console.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if path.startswith("/topics/") and len(path.split("/")) > 2:
        ...
        parts[2] = "{name}"
        path = "/".join(parts)
    if path.startswith("/consumer-groups/") and len(path.split("/")) > 2:
        parts = path.split("/")
        parts[2] = "{name}"
        path = "/".join(parts)
