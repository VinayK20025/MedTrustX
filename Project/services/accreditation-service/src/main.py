"""
MedTrustX Accreditation Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter("accreditation_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("accreditation_request_duration_seconds", "Request latency in seconds", ["method", "endpoint"])
...
app = FastAPI(
    title="MedTrustX Accreditation Service",
    description="Compliance framework lifecycle, checklists, evidence, and audits.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    for prefix in ("/accreditation/", "/standards/", "/checklists/", "/evidence/", "/audits/"):
        if prefix in path:
            parts = path.split("/")
            normalized = []
            for i, part in enumerate(parts):
                if i > 0 and parts[i - 1] in ("programs", "standards", "checklists", "evidence", "audits"):
...
from src.routes.accreditation import router as accreditation_router
