"""
MedTrustX Population Health Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter(
    "population_health_requests_total",
    "Total HTTP requests",
    ["method", "endpoint", "status"],
)
REQUEST_LATENCY = Histogram(
    "population_health_request_duration_seconds",
...
app = FastAPI(
    title="MedTrustX Population Health Service",
    description="Population-level intelligence — cohort segmentation, risk stratification, care gap detection, interventions.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    for prefix in ("/populations/", "/risk-profiles/"):
        if prefix in path:
            parts = path.split("/")
            normalized = []
            for i, part in enumerate(parts):
                if i > 0 and parts[i - 1] in ("populations", "risk-profiles"):
...
from src.routes.population import router as population_router
