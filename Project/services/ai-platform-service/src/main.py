"""
MedTrustX AI Platform Service — Application Entrypoint
"""
import time
...
app = FastAPI(
    ...
    description="Predictive and prescriptive intelligence — clinical, operational, financial, and security ML models.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    for prefix in ("/ai/models/", "/ai/predictions/"):
        if prefix in path:
            parts = path.split("/")
            normalized = []
            for i, part in enumerate(parts):
                if i > 0 and parts[i - 1] in ("models", "predictions"):
