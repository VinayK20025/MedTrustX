"""
MedTrustX OT Management Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter(
    "ot_service_requests_total",
    "Total HTTP requests",
    ["method", "endpoint", "status"],
)
REQUEST_LATENCY = Histogram(
    "ot_service_request_duration_seconds",
...
app = FastAPI(
    title="MedTrustX OT Management Service",
    description="Surgical scheduling, room allocation, and procedure lifecycle orchestration.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if "/surgeries" in path or "/ot-rooms" in path or "/ot-bookings" in path:
        parts = path.split("/")
        normalized = []
        for part in parts:
            try:
...
from src.routes.surgeries import router as surgeries_router
...
app.include_router(surgeries_router, prefix="/api/v1", tags=["Surgeries"])
