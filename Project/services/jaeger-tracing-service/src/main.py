"""
MedTrustX Jaeger Tracing Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter("jaeger_tracing_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("jaeger_tracing_request_duration_seconds", "Request latency in seconds", ["method", "endpoint"])
...
app = FastAPI(
    title="MedTrustX Jaeger Tracing Service",
    description="Tier-0 distributed tracing and latency mapping.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if path.startswith("/traces/") and len(path.split("/")) > 2:
        parts = path.split("/")
        parts[2] = "{trace_id}"
        path = "/".join(parts)
...
from src.routes.jaeger import router as jaeger_router

app.include_router(health_router, tags=["Health"])
app.include_router(jaeger_router)
