"""
MedTrustX Medical Records Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter(
    "medical_records_service_requests_total",
    "Total HTTP requests",
    ["method", "endpoint", "status"],
)
REQUEST_LATENCY = Histogram(
    "medical_records_service_request_duration_seconds",
...
app = FastAPI(
    title="MedTrustX Medical Records Service",
    description="Longitudinal medical history and document artifact persistence.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if "/medical-records" in path or "/patients/" in path:
        parts = path.split("/")
        normalized = []
        for part in parts:
            try:
...
app.include_router(records_router, prefix="/api/v1", tags=["Medical Records"])
app.include_router(documents_router, prefix="/api/v1", tags=["Documents"])
app.include_router(export_router, prefix="/api/v1", tags=["EHR Export"])
