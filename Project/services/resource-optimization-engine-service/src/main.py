"""
MedTrustX Resource Optimization Engine Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter("resource_optimization_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("resource_optimization_request_duration_seconds", "Request latency in seconds", ["method", "endpoint"])
...
app = FastAPI(
    title="MedTrustX Resource Optimization Engine Service",
    description="Tier-0 intelligent resource allocation and constraint-based optimization.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if path.startswith("/resources/") and len(path.split("/")) > 2:
        parts = path.split("/")
        parts[2] = "{id}"
        path = "/".join(parts)
    if path.startswith("/optimization/") and len(path.split("/")) > 2:
        parts = path.split("/")
        parts[2] = "{run_id}"
        path = "/".join(parts)
...
from src.routes.optimization import router as optimization_router
