"""
MedTrustX Infection Control Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter(
    "infection_service_requests_total",
    "Total HTTP requests",
    ["method", "endpoint", "status"],
)
REQUEST_LATENCY = Histogram(
    "infection_service_request_duration_seconds",
...
app = FastAPI(
    title="MedTrustX Infection Control Service",
    description="Epidemiological tracking, outbreak detection, and hygiene compliance.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if "/infections" in path or "/patients" in path:
        parts = path.split("/")
        normalized = []
        for part in parts:
            try:
...
from src.routes.infections import router as infections_router
from src.routes.isolation import router as isolation_router
from src.routes.audits import router as audits_router
from src.routes.resistance import router as resistance_router

app.include_router(health_router, tags=["Health"])
app.include_router(infections_router, prefix="/api/v1", tags=["Infections"])
app.include_router(isolation_router, prefix="/api/v1", tags=["Isolation"])
app.include_router(audits_router, prefix="/api/v1", tags=["Audits"])
app.include_router(resistance_router, prefix="/api/v1", tags=["Antimicrobial Resistance"])
