"""
MedTrustX Security Incident Response Service — Application Entrypoint
"""
import time, logging
...
REQUEST_COUNT = Counter("incident_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("incident_request_duration_seconds", "Request latency", ["method", "endpoint"])
...
app = FastAPI(title="MedTrustX Security Incident Response Service", description="Tier-0 physical security event handler.", version=settings.SERVICE_VERSION, docs_url="/docs", redoc_url="/redoc", openapi_url="/openapi.json", lifespan=lifespan)
...
async def metrics_middleware(request: Request, call_next):
    ...
    if path.startswith("/incidents/") and len(path.split("/")) > 2:
        parts = path.split("/")
        parts[2] = "{id}"
        if len(parts) > 3:
            parts[3] = "actions" if parts[3] == "actions" else "logs"
        path = "/".join(parts)
