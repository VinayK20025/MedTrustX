"""
MedTrustX Keycloak Shim Service — Application Entrypoint
"""
import time
...
app = FastAPI(
    title="MedTrustX Keycloak Shim Service",
    description="Identity Provider Integration and Token Shim.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if "/realms" in path:
        ...
        for i, part in enumerate(parts):
            if i > 0 and parts[i-1] == "realms":
                normalized.append("{realm}")
            else:
...
from src.routes.oidc import router as oidc_router
from src.routes.admin import router as admin_router

app.include_router(health_router, tags=["Health"])
# OIDC / Keycloak specific routes don't usually sit behind /api/v1
