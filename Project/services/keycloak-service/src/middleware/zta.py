...
_RESOURCE_MAP = {
    "/token": "idp_token",
    "/logout": "idp_session",
    "/userinfo": "idp_userinfo",
    "/certs": "idp_certs",
    "/users": "idp_admin_user",
}
...
class ZTAMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        # For public endpoints like /token and /certs, we might skip ZTA or use a specialized check.
        # But for architecture alignment, we evaluate.
        role = request.headers.get("X-User-Role", "anonymous")
        if "token" in request.url.path or "certs" in request.url.path:
            # Token generation usually starts unauthenticated if grant_type=password
            return await call_next(request)
