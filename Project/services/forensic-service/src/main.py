"""
MedTrustX Forensic Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter("for_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("for_request_duration_seconds", "Request latency in seconds", ["method", "endpoint"])
...
app = FastAPI(
    title="MedTrustX Forensic Service",
    description="Medico-legal cases and evidence management.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    for prefix in ("/forensic/cases/", "/evidence/", "/custody-logs/", "/forensic-reports/", "/external-requests/"):
        if prefix in path:
            parts = path.split("/")
            normalized = []
            for i, part in enumerate(parts):
                if i > 0 and parts[i - 1] in ("cases", "evidence", "custody-logs", "forensic-reports", "external-requests"):
...
from src.routes.forensic import router as forensic_router
