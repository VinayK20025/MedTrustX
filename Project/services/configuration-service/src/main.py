"""
MedTrustX Configuration Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter("configuration_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("configuration_request_duration_seconds", "Request latency in seconds", ["method", "endpoint"])
...
app = FastAPI(
    title="MedTrustX Configuration Service",
    description="Centralized configuration and runtime control plane.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    for prefix in ("/config/", "/feature-flags", "/environments", "/config/versions"):
        if prefix in path:
            parts = path.split("/")
            normalized = []
            for i, part in enumerate(parts):
                if i > 0 and parts[i - 1] in ("config", "feature-flags", "environments"):
                    if part != "versions":
...
from src.routes.configuration import router as configuration_router
