"""
MedTrustX Risk Management Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter("rm_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("rm_request_duration_seconds", "Request latency in seconds", ["method", "endpoint"])
...
app = FastAPI(
    title="MedTrustX Risk Management Service",
    description="Risk identification, scoring, and mitigation planning.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    for prefix in ("/risks/", "/mitigation-plans"):
        if prefix in path and not path.endswith("high"):
            parts = path.split("/")
            normalized = []
            for i, part in enumerate(parts):
                if i > 0 and parts[i - 1] in ("risks", "mitigation-plans"):
