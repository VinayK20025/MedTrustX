"""
MedTrustX Loki Logging Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter("loki_logging_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("loki_logging_request_duration_seconds", "Request latency in seconds", ["method", "endpoint"])
...
app = FastAPI(
    title="MedTrustX Loki Logging Service",
    description="Tier-0 Log aggregation and label-based indexing system.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if path.startswith("/logs/") and len(path.split("/")) > 2:
        parts = path.split("/")
        if parts[2] not in ["ingest", "query", "streams", "labels"]:
            parts[2] = "{id}"
        path = "/".join(parts)
...
from src.routes.loki import router as loki_router
