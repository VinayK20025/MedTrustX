"""
MedTrustX Regulator Integration Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter("ri_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("ri_request_duration_seconds", "Request latency in seconds", ["method", "endpoint"])
...
app = FastAPI(
    title="MedTrustX Regulator Integration Service",
...
async def metrics_middleware(request: Request, call_next):
    ...
    for prefix in ("/regulators/", "/reports/", "/submissions/", "/acknowledgments/"):
        if prefix in path and path != "/reports/submit":
            parts = path.split("/")
            normalized = []
            for i, part in enumerate(parts):
                if i > 0 and parts[i - 1] in ("regulators", "reports", "submissions", "acknowledgments"):
...
from src.routes.regulator import router as regulator_router
