"""
MedTrustX SCM Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter(
    "scm_service_requests_total",
    "Total HTTP requests",
    ["method", "endpoint", "status"],
)
REQUEST_LATENCY = Histogram(
    "scm_service_request_duration_seconds",
...
app = FastAPI(
    title="MedTrustX SCM Service",
    description="Supply Chain Management, Procurement, and Vendor Orchestration.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if "/vendors" in path or "/purchase-orders" in path or "/goods-receipts" in path or "/shipments" in path:
        parts = path.split("/")
        normalized = []
        for part in parts:
            try:
...
from src.routes.goods_receipts import router as receipt_router
from src.routes.shipments import router as shipment_router
