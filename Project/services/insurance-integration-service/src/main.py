"""
MedTrustX Insurance Integration Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter("ii_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("ii_request_duration_seconds", "Request latency in seconds", ["method", "endpoint"])
...
app = FastAPI(
    title="MedTrustX Insurance Integration Service",
    description="Insurance orchestration layer.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    for prefix in ("/insurance/eligibility/", "/insurance/preauth/", "/insurance/claims/"):
        if prefix in path and not path.endswith("status"):
            parts = path.split("/")
            normalized = []
            for i, part in enumerate(parts):
                if i > 1 and parts[i - 1] in ("eligibility", "preauth", "claims"):
