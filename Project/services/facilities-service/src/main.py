"""
MedTrustX Facilities Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter(
    "facilities_service_requests_total",
    "Total HTTP requests",
    ["method", "endpoint", "status"],
)
REQUEST_LATENCY = Histogram(
    "facilities_service_request_duration_seconds",
...
app = FastAPI(
    title="MedTrustX Facilities Service",
    description="Physical infrastructure, asset management, and maintenance.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if "/facilities" in path or "/rooms" in path or "/assets" in path or "/maintenance" in path:
        parts = path.split("/")
        normalized = []
        for part in parts:
            try:
...
from src.routes.facilities import router as facilities_router
from src.routes.rooms import router as rooms_router
from src.routes.assets import router as assets_router
from src.routes.maintenance import router as maintenance_router
