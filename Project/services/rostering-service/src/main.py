"""
MedTrustX Rostering Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter("rostering_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("rostering_request_duration_seconds", "Request latency in seconds", ["method", "endpoint"])
...
app = FastAPI(
    title="MedTrustX Rostering Service",
    description="Workforce scheduling, shifts, assignments, and availability.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    for prefix in ("/schedules/", "/shifts/", "/assignments/", "/availability/", "/leaves/"):
        if prefix in path:
            parts = path.split("/")
            normalized = []
            for i, part in enumerate(parts):
                if i > 0 and parts[i - 1] in ("schedules", "shifts", "assignments", "availability", "leaves"):
...
from src.routes.rostering import router as rostering_router
