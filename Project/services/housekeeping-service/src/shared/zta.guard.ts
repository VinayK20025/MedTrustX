...
export class ZtaGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    let resource = 'hk_resource';
    if (request.path.includes('/housekeeping/tasks')) resource = 'hk_tasks';
    else if (request.path.includes('/room-status')) resource = 'hk_room_status';
    else if (request.path.includes('/sanitation-logs')) resource = 'hk_sanitation';
    else if (request.path.includes('/waste-management')) resource = 'hk_waste';
    else if (request.path.includes('/housekeeping/events')) resource = 'hk_events';
