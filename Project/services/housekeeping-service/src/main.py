"""
MedTrustX Housekeeping Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter("hk_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("hk_request_duration_seconds", "Request latency in seconds", ["method", "endpoint"])
...
app = FastAPI(
    title="MedTrustX Housekeeping Service",
    description="Facility cleanliness, room turnarounds, waste management.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    for prefix in ("/housekeeping/tasks/", "/room-status/", "/sanitation-logs", "/waste-management"):
        if prefix in path:
            parts = path.split("/")
            normalized = []
            for i, part in enumerate(parts):
                if i > 0 and parts[i - 1] in ("tasks", "room-status"):
...
from src.routes.housekeeping import router as housekeeping_router
