"""
MedTrustX Compliance Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter(
    "comp_service_requests_total",
    "Total HTTP requests", ["method", "endpoint", "status"],
)
REQUEST_LATENCY = Histogram(
    "comp_service_request_duration_seconds",
...
app = FastAPI(
    ...
    description="Regulatory adherence, consent management, and policy enforcement.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if any(p in path for p in ["/consents", "/policies", "/legal-documents", "/compliance-checks", "/violations"]):
        parts = path.split("/")
        normalized = []
        for part in parts:
            try:
...
from src.routes.consents import router as consents_router
from src.routes.policies import router as policies_router
from src.routes.documents import router as documents_router
from src.routes.checks import router as checks_router
from src.routes.violations import router as violations_router
