"""
MedTrustX Clinical Pathway Intelligence Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter("clinical_pathway_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("clinical_pathway_request_duration_seconds", "Request latency in seconds", ["method", "endpoint"])
...
app = FastAPI(
    title="MedTrustX Clinical Pathway Intelligence Service",
    description="Tier-0 clinical decision support and pathway optimization engine.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if path.startswith("/pathways/") and len(path.split("/")) > 2:
        parts = path.split("/")
        parts[2] = "{id}"
        path = "/".join(parts)
    if path.startswith("/journeys/") and len(path.split("/")) > 2:
        parts = path.split("/")
        parts[2] = "{patient_id}"
        path = "/".join(parts)
...
from src.routes.pathway import router as pathway_router
