"""
MedTrustX HR Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter(
    "hr_service_requests_total",
    "Total HTTP requests",
    ["method", "endpoint", "status"],
)
REQUEST_LATENCY = Histogram(
    "hr_service_request_duration_seconds",
...
app = FastAPI(
    title="MedTrustX HR Service",
    description="Workforce management, staff roles, and organizational structure.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if "/employees" in path or "/departments" in path or "/roles" in path:
        parts = path.split("/")
        normalized = []
        for part in parts:
            try:
