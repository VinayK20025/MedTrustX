"""
MedTrustX Credentialing Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter("credentialing_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("credentialing_request_duration_seconds", "Request latency in seconds", ["method", "endpoint"])
...
app = FastAPI(
    title="MedTrustX Credentialing Service",
    description="Provider credentials, privileges, verifications, and workflows.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    for prefix in ("/credentials/", "/privileges/", "/privileging-requests/", "/users/"):
        if prefix in path:
            parts = path.split("/")
            normalized = []
            for i, part in enumerate(parts):
                if i > 0 and parts[i - 1] in ("credentials", "privileges", "privileging-requests", "users"):
...
from src.routes.credentialing import router as credentialing_router

app.include_router(health_router, tags=["Health"])
app.include_router(credentialing_router)
