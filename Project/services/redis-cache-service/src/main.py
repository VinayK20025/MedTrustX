"""
MedTrustX Redis Cache Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter("redis_cache_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("redis_cache_request_duration_seconds", "Request latency in seconds", ["method", "endpoint"])
...
app = FastAPI(
    title="MedTrustX Redis Cache Service",
    description="High-performance in-memory data, caching, and rate limiting.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if path.startswith("/cache/") and len(path.split("/")) > 2:
        parts = path.split("/")
        if parts[2] != "set":
            parts[2] = "{key}"
        path = "/".join(parts)
