"""
MedTrustX Edge Connectivity Manager Service — Application Entrypoint
"""
import time, logging
...
REQUEST_COUNT = Counter("edge_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("edge_request_duration_seconds", "Request latency", ["method", "endpoint"])
...
app = FastAPI(title="MedTrustX Edge Connectivity Manager Service", description="Tier-0 edge resilience and link management.", version=settings.SERVICE_VERSION, docs_url="/docs", redoc_url="/redoc", openapi_url="/openapi.json", lifespan=lifespan)
...
from src.routes.edge import router as edge_router
