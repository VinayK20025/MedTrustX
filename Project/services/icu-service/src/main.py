"""
MedTrustX ICU Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter(
    "icu_service_requests_total",
    "Total HTTP requests",
    ["method", "endpoint", "status"],
)
REQUEST_LATENCY = Histogram(
    "icu_service_request_duration_seconds",
...
app = FastAPI(
    title="MedTrustX ICU Service",
    description="Real-time intensive care monitoring, device integration, and alerting.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if "/icu/" in path or "/patients/" in path:
        parts = path.split("/")
        normalized = []
        for part in parts:
            try:
...
app.include_router(admissions_router, prefix="/api/v1", tags=["Admissions"])
app.include_router(vitals_router, prefix="/api/v1", tags=["Vitals Stream"])
app.include_router(devices_router, prefix="/api/v1", tags=["Devices"])
app.include_router(alerts_router, prefix="/api/v1", tags=["Critical Alerts"])
