"""
MedTrustX Role Management Service — Application Entrypoint
"""
import time
...
app = FastAPI(
    title="MedTrustX Role Management Service",
    description="Business-context roles, permission mappings, and hierarchies.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if any(p in path for p in ["/roles", "/permissions", "/users"]):
        parts = path.split("/")
        normalized = []
        for part in parts:
            try:
...
from src.routes.hierarchy import router as hierarchy_router
from src.routes.user_roles import router as user_roles_router
