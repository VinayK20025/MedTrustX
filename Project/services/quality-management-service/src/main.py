"""
MedTrustX Quality Management Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter("quality_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("quality_request_duration_seconds", "Request latency in seconds", ["method", "endpoint"])
...
app = FastAPI(
    title="MedTrustX Quality Management Service",
    description="Quality metrics, incidents, RCA, and compliance audits.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    for prefix in ("/quality/", "/incidents/", "/audits/", "/improvement-plans/"):
        if prefix in path:
            parts = path.split("/")
            normalized = []
            for i, part in enumerate(parts):
                if i > 0 and parts[i - 1] in ("incidents", "audits"):
