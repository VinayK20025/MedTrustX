"""
MedTrustX Clinical Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter(
    "clinical_service_requests_total",
    "Total HTTP requests",
    ["method", "endpoint", "status"],
)
REQUEST_LATENCY = Histogram(
    "clinical_service_request_duration_seconds",
...
app = FastAPI(
    title="MedTrustX Clinical Records Service",
    description="Core medical intelligence layer. Manages encounters, clinical notes, diagnoses, and observations.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if "/encounters/" in path or "/patients/" in path:
        parts = path.split("/")
        normalized = []
        for part in parts:
            try:
...
from src.routes.encounters import router as encounters_router
from src.routes.notes import router as notes_router
from src.routes.diagnoses import router as diagnoses_router
...
app.include_router(notes_router, prefix="/api/v1", tags=["Clinical Notes"])
...
app.include_router(history_router, prefix="/api/v1", tags=["Clinical History"])
