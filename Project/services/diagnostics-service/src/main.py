"""
MedTrustX Diagnostics Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter(
    "diagnostics_service_requests_total",
    "Total HTTP requests",
    ["method", "endpoint", "status"],
)
REQUEST_LATENCY = Histogram(
    "diagnostics_service_request_duration_seconds",
...
app = FastAPI(
    title="MedTrustX Diagnostics Service",
    description="Manages diagnostic investigations, lab results, and imaging studies workflows.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if "/orders/" in path or "/patients/" in path:
        parts = path.split("/")
        normalized = []
        for part in parts:
            try:
...
from src.routes.samples import router as samples_router
from src.routes.results import router as results_router
from src.routes.imaging import router as imaging_router
