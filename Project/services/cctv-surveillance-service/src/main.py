"""
MedTrustX CCTV & Surveillance Service — Application Entrypoint
"""
import time, logging
...
REQUEST_COUNT = Counter("cctv_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("cctv_request_duration_seconds", "Request latency", ["method", "endpoint"])
...
app = FastAPI(title="MedTrustX CCTV & Surveillance Service", description="Tier-0 real-time video monitoring layer.", version=settings.SERVICE_VERSION, docs_url="/docs", redoc_url="/redoc", openapi_url="/openapi.json", lifespan=lifespan)
...
async def metrics_middleware(request: Request, call_next):
    ...
    if path.startswith("/cameras/") and len(path.split("/")) > 2:
        parts = path.split("/")
        parts[2] = "{id}"
        path = "/".join(parts)
    elif path.startswith("/streams/") and len(path.split("/")) > 2:
        parts = path.split("/")
        parts[2] = "{camera_id}"
        path = "/".join(parts)
...
from src.routes.cctv import router as cctv_router
