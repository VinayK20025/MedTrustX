"""
MedTrustX Appointments Service — Application Entrypoint
"""
import time
...
app = FastAPI(
    title="MedTrustX Appointments Service",
    description="High-volume clinician scheduling and patient booking engine.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if "/schedules" in path or "/appointments" in path or "/doctors" in path or "/patients" in path or "/queues" in path:
        parts = path.split("/")
        normalized = []
        for part in parts:
            try:
...
from src.routes.queues import router as queues_router
