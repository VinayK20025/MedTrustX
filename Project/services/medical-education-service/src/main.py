"""
MedTrustX Medical Education Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter("medical_education_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("medical_education_request_duration_seconds", "Request latency in seconds", ["method", "endpoint"])
...
app = FastAPI(
    title="MedTrustX Medical Education Service",
    description="CME, simulation training, assessments, and certification management.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    for prefix in ("/courses/", "/assessments/", "/certifications/"):
        if prefix in path:
            parts = path.split("/")
            normalized = []
            for i, part in enumerate(parts):
                if i > 0 and parts[i - 1] in ("courses", "assessments", "certifications"):
...
from src.routes.education import router as education_router
