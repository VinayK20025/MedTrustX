"""
MedTrustX Fire & Safety Systems Service — Application Entrypoint
"""
import time, logging
...
REQUEST_COUNT = Counter("safety_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("safety_request_duration_seconds", "Request latency", ["method", "endpoint"])
...
app = FastAPI(title="MedTrustX Fire & Safety Systems Service", description="Tier-0 critical safety monitoring layer.", version=settings.SERVICE_VERSION, docs_url="/docs", redoc_url="/redoc", openapi_url="/openapi.json", lifespan=lifespan)
