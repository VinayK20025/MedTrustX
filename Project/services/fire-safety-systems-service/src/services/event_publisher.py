"""
MedTrustX Fire & Safety Systems Service — Event Publisher
"""
import json
...
TOPIC = "medtrust.safety.events"

class SafetyEventEncoder(json.JSONEncoder):
...
async def init_producer() -> None:
    global _producer
    try:
        _producer = AIOKafkaProducer(bootstrap_servers=settings.KAFKA_BROKERS, value_serializer=lambda v: json.dumps(v, cls=SafetyEventEncoder).encode("utf-8"), key_serializer=lambda k: k.encode("utf-8") if k else None, acks="all", enable_idempotence=True, max_request_size=1_048_576)
