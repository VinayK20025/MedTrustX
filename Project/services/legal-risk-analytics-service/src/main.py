"""
MedTrustX Legal Risk Analytics Service — Application Entrypoint
"""
import time, logging
...
REQUEST_COUNT = Counter("legalrisk_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("legalrisk_request_duration_seconds", "Request latency", ["method", "endpoint"])
...
app = FastAPI(title="MedTrustX Legal Risk Analytics Service", description="Tier-0 legal intelligence and risk evaluation.", version=settings.SERVICE_VERSION, docs_url="/docs", redoc_url="/redoc", openapi_url="/openapi.json", lifespan=lifespan)
...
async def metrics_middleware(request: Request, call_next):
    ...
    if path.startswith("/risk/") and len(path.split("/")) > 2 and path.split("/")[2] != "evaluate":
        parts = path.split("/")
        parts[2] = "{id}"
        path = "/".join(parts)
