"""
MedTrustX Strategic Planning Service — Event Publisher
"""
import json
...
TOPIC = "medtrust.planning.events"

class PlanningEventEncoder(json.JSONEncoder):
...
async def init_producer() -> None:
    global _producer
    try:
        _producer = AIOKafkaProducer(bootstrap_servers=settings.KAFKA_BROKERS, value_serializer=lambda v: json.dumps(v, cls=PlanningEventEncoder).encode("utf-8"), key_serializer=lambda k: k.encode("utf-8") if k else None, acks="all", enable_idempotence=True, max_request_size=1_048_576)
