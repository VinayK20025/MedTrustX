"""
MedTrustX Marketing Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter(
    "mktg_service_requests_total",
    "Total HTTP requests", ["method", "endpoint", "status"],
)
REQUEST_LATENCY = Histogram(
    "mktg_service_request_duration_seconds",
...
app = FastAPI(
    title="MedTrustX Marketing Service",
    description="Data-driven engagement, segmentation, and campaign tracking.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if any(p in path for p in ["/campaigns", "/segments", "/engagement-events", "/funnels"]):
        parts = path.split("/")
        normalized = []
        for part in parts:
            try:
...
from src.routes.segments import router as segments_router
from src.routes.engagement import router as engagement_router
from src.routes.funnels import router as funnels_router
