"""
MedTrustX Transplant Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter(
    "transplant_service_requests_total",
    "Total HTTP requests", ["method", "endpoint", "status"],
)
REQUEST_LATENCY = Histogram(
    "transplant_service_request_duration_seconds",
...
app = FastAPI(
    title="MedTrustX Transplant Service",
    description="Organ allocation and transplant coordination engine.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if any(p in path for p in ["/donors", "/recipients", "/waitlists", "/matches", "/transplants"]):
        parts = path.split("/")
        normalized = []
        for part in parts:
            try:
...
from src.routes.recipients import router as recipients_router
from src.routes.waitlists import router as waitlists_router
from src.routes.matches import router as matches_router
from src.routes.transplants import router as transplants_router

app.include_router(health_router, tags=["Health"])
app.include_router(donors_router, prefix="/api/v1")
app.include_router(recipients_router, prefix="/api/v1")
app.include_router(waitlists_router, prefix="/api/v1")
app.include_router(matches_router, prefix="/api/v1")
app.include_router(transplants_router, prefix="/api/v1")
...
async def global_exception_handler(request: Request, exc: Exception):
    ...
    return JSONResponse(
        status_code=500,
        content={"error": "internal_server_error", "message": "An unexpected error occurred", "service": settings.SERVICE_NAME},
    )
