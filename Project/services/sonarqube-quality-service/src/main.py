"""
MedTrustX SonarQube Quality Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter("sonarqube_quality_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("sonarqube_quality_request_duration_seconds", "Request latency in seconds", ["method", "endpoint"])
...
app = FastAPI(
    title="MedTrustX SonarQube Quality Service",
    description="Tier-1 static analysis and quality gate enforcement.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if path.startswith("/projects/") and len(path.split("/")) > 2:
        parts = path.split("/")
        parts[2] = "{id}"
        path = "/".join(parts)
...
from src.routes.sonarqube import router as sonarqube_router
