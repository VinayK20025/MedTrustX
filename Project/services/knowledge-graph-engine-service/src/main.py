"""
MedTrustX Knowledge Graph Engine Service — Application Entrypoint
"""
import time, logging
...
REQUEST_COUNT = Counter("knowledge_graph_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("knowledge_graph_request_duration_seconds", "Request latency", ["method", "endpoint"])
...
app = FastAPI(title="MedTrustX Knowledge Graph Engine Service", description="Tier-0 semantic modeling and relational insights.", version=settings.SERVICE_VERSION, docs_url="/docs", redoc_url="/redoc", openapi_url="/openapi.json", lifespan=lifespan)
...
async def metrics_middleware(request: Request, call_next):
    ...
    if path.startswith("/nodes/") and len(path.split("/")) > 2:
