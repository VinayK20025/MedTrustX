"""
MedTrustX Postal Mail Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter("postal_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("postal_request_duration_seconds", "Request latency in seconds", ["method", "endpoint"])
...
app = FastAPI(
    title="MedTrustX Postal Mail Service",
    description="Scalable infrastructure for outbound transactional emails.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if path.startswith("/emails/") and len(path.split("/")) > 2:
        parts = path.split("/")
        if parts[2] not in ["send", "bounces"]:
            parts[2] = "{id}"
        path = "/".join(parts)
...
from src.routes.postal import router as postal_router
