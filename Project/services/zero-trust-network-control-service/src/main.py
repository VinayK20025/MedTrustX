"""
MedTrustX Zero Trust Network Control Service — Application Entrypoint
"""
import time, logging
...
REQUEST_COUNT = Counter("ztnetwork_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("ztnetwork_request_duration_seconds", "Request latency", ["method", "endpoint"])
...
app = FastAPI(title="MedTrustX Zero Trust Network Control Service", description="Tier-0 network security enforcement layer.", version=settings.SERVICE_VERSION, docs_url="/docs", redoc_url="/redoc", openapi_url="/openapi.json", lifespan=lifespan)
...
async def metrics_middleware(request: Request, call_next):
    ...
    if path.startswith("/sessions/") and len(path.split("/")) > 2:
        parts = path.split("/")
        parts[2] = "{id}"
        path = "/".join(parts)
...
from src.routes.zt_network import router as zt_router
