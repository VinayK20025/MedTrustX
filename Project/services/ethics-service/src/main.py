"""
MedTrustX Ethics Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter("ethics_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("ethics_request_duration_seconds", "Request latency in seconds", ["method", "endpoint"])
...
app = FastAPI(
    title="MedTrustX Ethics Service",
    description="Ethics cases, committee workflows, reviews, and conflict declarations.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    for prefix in ("/ethics/cases/", "/ethics/committee", "/ethics/conflicts", "/ethics/policies"):
        if prefix in path:
            parts = path.split("/")
            normalized = []
            for i, part in enumerate(parts):
                if i > 0 and parts[i - 1] == "cases":
...
from src.routes.ethics import router as ethics_router
