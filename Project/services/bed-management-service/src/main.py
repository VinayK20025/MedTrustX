"""
MedTrustX Bed Management Service — Application Entrypoint

Tier-0/Tier-1 Hybrid Service — controls real-time bed allocation, tracking,
and optimization across wards, ICU, ER, and specialty units.

Manages: bed inventory, allocations, reservations, transfers,
bed turnover lifecycle, and capacity dashboards.
"""
import time
...
REQUEST_COUNT = Counter(
    "bed_mgmt_service_requests_total",
    "Total HTTP requests", ["method", "endpoint", "status"],
)
REQUEST_LATENCY = Histogram(
    "bed_mgmt_service_request_duration_seconds",
...
app = FastAPI(
    title="MedTrustX Bed Management Service",
    description=(
        "Real-time bed allocation, tracking, and optimization across "
        "wards, ICU, ER, and specialty units. Manages bed inventory, "
        "patient admissions, transfers, reservations, and capacity dashboards."
    ),
    version=settings.SERVICE_VERSION,
...
async def metrics_middleware(request: Request, call_next):
    ...
    if "/beds" in path or "/bed-" in path:
        parts = path.split("/")
        normalized = []
        for part in parts:
            try:
...
from src.routes.beds import router as beds_router
...
# IMPORTANT: availability must be registered before beds to avoid
# /beds/{id} matching /beds/availability
...
async def global_exception_handler(request: Request, exc: Exception):
    logger.error("unhandled_exception", path=request.url.path,
                 method=request.method, error=str(exc), exc_info=True)
    return JSONResponse(
        status_code=500,
        content={
