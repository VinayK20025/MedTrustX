"""
MedTrustX Bed Management Service — Event Publisher

Publishes bed lifecycle events to Redpanda/Kafka.

Events: BED_ALLOCATED, BED_RELEASED, BED_TRANSFERRED,
        BED_STATUS_UPDATED, BED_RESERVED, BED_RESERVATION_CANCELLED
"""
import json
...
TOPIC = "medtrust.bed.events"


class BedEventEncoder(json.JSONEncoder):
...
async def init_producer() -> None:
    ...
    try:
        _producer = AIOKafkaProducer(
            ...
            value_serializer=lambda v: json.dumps(v, cls=BedEventEncoder).encode("utf-8"),
