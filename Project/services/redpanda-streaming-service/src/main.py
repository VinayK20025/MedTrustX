"""
MedTrustX Redpanda Streaming Service — Application Entrypoint
"""
import time
...
REQUEST_COUNT = Counter("redpanda_streaming_requests_total", "Total HTTP requests", ["method", "endpoint", "status"])
REQUEST_LATENCY = Histogram("redpanda_streaming_request_duration_seconds", "Request latency in seconds", ["method", "endpoint"])
...
app = FastAPI(
    title="MedTrustX Redpanda Streaming Service",
    description="Tier-0 Kafka-compatible event streaming and topic metadata registry.",
...
async def metrics_middleware(request: Request, call_next):
    ...
    if path.startswith("/topics/") and len(path.split("/")) > 2:
        parts = path.split("/")
        parts[2] = "{id}"
        path = "/".join(parts)
...
from src.routes.streaming import router as streaming_router
