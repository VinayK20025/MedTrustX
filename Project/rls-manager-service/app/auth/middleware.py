"""
Auth Middleware.
Validates PQC+JWT on protected endpoints and attaches claims to request.state.
"""
...
class PQCAuthMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next: RequestResponseEndpoint):
        if not request.url.path.startswith("/api/") or request.url.path.startswith("/api/docs") or request.url.path.startswith("/api/openapi"):
        ...
        try:
            ...
            # Extract roles from realm_access or directly
